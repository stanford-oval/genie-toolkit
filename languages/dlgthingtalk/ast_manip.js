...
function makeListExpression(param, filter=null) {
    if (filter) {
...
function makeAggregateFilter(param, aggregationOp, field, op, value) {
    const list = makeListExpression(param);
    if (!list)
        return null;
...
function makeProjection(table, pname) {
    ...
    return new Ast.Table.Projection(table, [pname], newSchema);
}
...
function makeSingleFieldProjection(ftype, ptype, table, pname) {
    ...
    if (ftype === 'table') {
        ...
        return new Ast.Stream.Projection(stream, [pname], newSchema);
    }
}
