...
async function postprocess(schemas : ThingTalk.SchemaRetriever, 
    ...
    const rlExamples = readline.createInterface({ input: input[0] });
    const rlBootleg = readline.createInterface({ input: input[1] });
    for await (const [example, bootleg] of zip(rlExamples, rlBootleg)) {
        ...
        if (qids.length === 0) {
            ...
            output[0].write(updatedExample + '\n');
            output[1].write(bootleg + '\n');
...
export async function execute(args : any) {
    ...
    await postprocess(schemas, 
                      ...
                      [args.examples, args.bootleg_output], 
                      [args.updated_examples, args.updated_bootleg_output]);
}