...
function ThingEngineFSAdapter() {
    this.fs = require('fs');
    this.path = require('path');
}

ThingEngineFSAdapter.prototype.loadDatabase = function(dbname, callback) {
    this.fs.readFile(path.join(platform.getWritableDir(), dbname), {
        encoding: 'utf8'
...
ThingEngineFSAdapter.prototype.saveDatabase = function(dbname, dbstring, callback) {
    this.fs.writeFile(path.join(platform.getWritableDir(), dbname), dbstring, callback);
};
...
function OmTable(client, name, key) {
	...
	if(persist) {
		...
		lokiSettings.adapter = new lokiIndexed("omclient");
                ...
                lokiSettings.adapter = new ThingEngineFSAdapter();
    }
