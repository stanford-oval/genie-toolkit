...
function genPubKey(data) {
	...
	, skey = new Uint8Array(data.skey);
	self.postMessage({ logMsg: "Do calculation of a public key for a given secret key.\n"
		+"Calculations are performed "+numOfRuns+" times, to provide an average time." });
		var millis = Date.now();
	for (var i=0; i<numOfRuns; i+=1) {
		ecmaNacl.box.generate_pubkey(skey);
	}
	millis = (Date.now() - millis)/numOfRuns;
	self.postMessage({
		logMsg: "\taverage: " + millis.toFixed(3) + " milliseconds",
		done: true });
}
...
function boxEnc(data) {
	...
	millis = (Date.now() - millis)/numOfRuns;
	self.postMessage({ logMsg: "\taverage for packing: "+millis.toFixed(3)+" milliseconds" });
	millis = Date.now();
	for ( var i = 0; i < numOfRuns; i += 1) {
	...
	self.postMessage({
		logMsg: "\taverage for opening: "+millis.toFixed(3)+" milliseconds",
		done: true });
}
