    ...
    if (primType === 'all') {
        ...
        if (streams.length === 0) {
            ...
            streams = streams.map((ex) => {
                ...
                stream.utterance = dlg.interpolate(
                    dlg._("when ${utterance} change notify me"),
        ...
        examples.forEach((ex) => {
            if (ex.type === 'query')
                ex.utterance = dlg.interpolate(dlg._("get ${utterance}"), { utterance: ex.utterance });
            if (ex.type === 'stream')
                ex.utterance = dlg.interpolate(dlg._("${utterance} notify me"), { utterance: ex.utterance });
            return ex;
        });
    } else {
        ...
        if (examples.length === 0 && primType === 'stream') {
            ...
            examples.forEach((ex) => {
                ex.utterance = dlg.interpolate(dlg._("when ${utterance} change"), { utterance: ex.utterance });
            });
        }
    }
    if (examples.length === 0) {
        await dlg.replyInterp(dlg._("Can't find a compatible command from ${device}, choose another device."), {
            device: Helpers.cleanKind(name)
        });
        await dlg.replySpecial(dlg._("Back"), 'back');
        return;
    }
...
function primitiveFromProgram(program, primType) {
    ...
    if (primType === 'stream' && rule.table)
        return new Ast.Stream.Monitor(null, rule.table, null, rule.table.schema);
...
function makeProgram(stream, query, action, executor=null) {
    let rule;
    if (action)
        action = new ThingTalk.Ast.Action.Invocation(null, action, null);
    else
        action = ThingTalk.Generate.notifyAction(executor ? 'return' : 'notify');
    if (stream && query)
        rule = new Ast.Statement.Rule(null, new Ast.Stream.Join(null, stream, query, [], null), [action]);
    else if (stream)
        rule = new Ast.Statement.Rule(null, stream, [action]);
    else
        rule = new Ast.Statement.Command(null, query, [action]);
    return new Ast.Program(null, [], [], [rule], executor);
}
...
module.exports = async function makeDialog(dlg, startIntent) {
    ...
    if (ENABLE_WHEN_GET_DO) {
        for (;;) {
            ...
            let choices = [
                utterances[0] ?
                    dlg.interpolate(dlg._("When: ${utterance}${filter}"), {
                        utterance: utterances[0],
                        filter:  filterDescription[0]
                    })
                    : dlg._("When"),
                utterances[1] ?
                    dlg.interpolate(dlg._("Get: ${utterance}${filter}"), {
                        utterance: utterances[1],
                        filter: filterDescription[1]
                    })
                    : dlg._("Get"),
                utterances[2] ?
                    dlg.interpolate(dlg._("Do: ${utterance}${filter}"), {
                        utterance: utterances[2],
                        filter: filterDescription[2]
                    })
                    : dlg._("Do")
            ];
            if (ready) {
            ...
            if (index === 3) {
                ...
                if (filterCandidates[0].length > 0) {
                    filterCommandChoices.push(['stream', dlg.interpolate(dlg._("When: ${utterance}${filter}"), {
                        utterance: utterances[0],
                        filter: filterDescription[0]
                    })]);
                }
                if (filterCandidates[1].length > 0) {
                    filterCommandChoices.push(['query', dlg.interpolate(dlg._("Get: ${utterance}${filter}"), {
                        utterance: utterances[1],
                        filter: filterDescription[1]
                    })]);
                }
        ...
        for (;;) {
            ...
            if (ready) {
                ...
                if (primType === 'stream' && !utterances[index].endsWith('notify me'))
                    format = dlg._("${utterance}${filter} notify me");
                else if (primType === 'query' && !utterances[index].startsWith('get'))
                    format = dlg._("get ${utterance}${filter}");
                else
                    format = "${utterance}${filter}";
                const description = dlg.interpolate(format, {
                    utterance: utterances[index],
                    filter: filterDescription[index]
                });
                choices.push(dlg._("Choose a different command"));
                if (primType !== 'action')
                    choices.push(dlg._("Add a filter"));
                choices.push(dlg._("Run it"));
                hint = dlg.interpolate(
                    dlg._("Your command is: ${command}. You can add more filters or run your command if you are ready."),
                    { command: description });
    ...
    if (rule.stream !== null && filters[0] && filters[0].length > 0)
        rule.stream = new Ast.Stream.Filter(null, rule.stream, new Ast.BooleanExpression.And(null, filters[0]), rule.stream.schema);
    if (rule.query !== null && filters[1] && filters[1].length > 0)
        rule.query = new Ast.Table.Filter(null, rule.query, new Ast.BooleanExpression.And(null, filters[1]), rule.query.schema);
