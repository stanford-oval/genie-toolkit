...
async function computePrediction(dlg, policy, state, intent) {
    ...
    const forceNewStyleDialogue = sharedPrefs.get('experimental-contextual-model');
    if (forceNewStyleDialogue && intent.isProgram) {
...
async function handleUserInput(dlg, state, input) {
    ...
    const sharedPrefs = platform.getSharedPreferences();
    const forceNewStyleDialogue = sharedPrefs.get('experimental-contextual-model');
    if (!input.intent.isDialogueState && (!forceNewStyleDialogue || !input.intent.isProgram)) {
