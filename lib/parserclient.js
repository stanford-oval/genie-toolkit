...
module.exports = class ParserClient {
    ...
    onlineLearn(utterance, code) {
        var url = this._baseUrl + '/learn?&q=' + encodeURIComponent(utterance)
            + '&target=' + encodeURIComponent(code.join(','));
    ...
    sendUtterance(utterance, expecting, choices) {
        var url = this._baseUrl + '/query?q=' + encodeURIComponent(utterance);
        if (expecting)
