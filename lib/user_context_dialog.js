...
module.exports = class UserContextDialog extends Dialog {
    ...
    static resolve(parent, slot, ctxVar) {
        ...
        if (parent.subdialog === null) {
            parent.push(new UserContextDialog(ctxVar));
            return parent.subdialog.continue().then((waiting) => {
                if (waiting) {
                    return waiting;
                } else {
                    slot.value = parent.subdialog.resolved;
                    parent.pop();
                    return false;
                }
            });
    ...
    _tryGetCurrentLocation() {
        var phone = this.manager.devices.getDevice('org.thingpedia.builtin.thingengine.phone');
        if (phone === null) {
    ...
    _valueFromJs(category, value) {
        if (category.isLocation)
            return Ast.Value.Location(Ast.Location.Absolute(value.x, value.y, value.display||null));
        else
