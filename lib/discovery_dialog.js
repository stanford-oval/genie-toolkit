...
module.exports = class DiscoveryDialog extends Dialog {
    ...
    needDiscovery() {
        var setup = this.manager._engine.thingpedia.getDeviceSetup(new Array(this.name));
        if (setup.type === 'link' || setup.type === 'oauth2') {
            return false;
        }
        return true;
    }
    ...
    _continue() {
        ...
        if (devices.length === 1) {
            ...
            this.ask(ValueCategory.YesNo, "Found a " + this.name + ". Do you want to set it up now?");
            return true;
        }
        if (devices.length > 1) {
