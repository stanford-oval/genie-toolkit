...
module.exports = class DiscoveryDialog extends Dialog {
    ...
    setDevice() {
        //TODO: get any device even the user don't have it
        this.device = this.manager.devices.getAllDevicesOfKind(this.name);
    }
        ...
        this.needDiscovery().then((need) => {
            if (need) {
                ...
                if (this.device.length !== 0) {
                    ...
                    this.reply(this._("So sorry that I can't find %s in my database.").format(this.name));
                    this.switchToDefault();
                    return false;
                }
            }
