...
module.exports = {
    ...
    computeUserPrediction(oldState, newState) {
        ...
        return newState.prettyprint().split(' ');
    },
     ...
     * Neither `oldState` nor `newState` must be modified in-place.
     *
     * @param {any} oldState - the previous dialogue state, before the system speaks
     * @param {any} newState - the new state of the dialogue, after the system speaks
     * @return {string[]} - the delta to predict, as a sequence of tokens
     */
    computeSystemPrediction(oldState, newState) {
        // we don't care about the agent dialogue policy in this task
        return [];
    },
