...
module.exports = class GenieEntityRetriever extends NNSyntax.EntityRetriever {
    ...
    _findEntityFromSentence(entityType, entityString, ignoreNotFound) {
        ...
        if (found)
            return this._tokenizer.detokenize(entityTokens);
        ...
        if (this._useHeuristics) {
            if (entityType === 'LOCATION') {
                ...
                if (entityString.indexOf(',') >= 0) {
                    ...
                    if (this._sentenceContains(entityNoComma))
                        return this._tokenizer.detokenize(entityNoComma);
                }
            ...
            const entityPlural = this._langPack.pluralize(this._tokenizer.detokenize(entityTokens));
            ...
            if (entityPlural && this._sentenceContains(this._tokenizer.tokenize(entityPlural).rawTokens))
                return this._tokenizer.detokenize(entityTokens);
        }
