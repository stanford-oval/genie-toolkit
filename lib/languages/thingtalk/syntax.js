...
export async function parse(code, options) {
...
export async function parsePrediction(code, entities, options) {
    const tpClient = options.thingpediaClient;
    if (!options.schemaRetriever)
        options.schemaRetriever = new SchemaRetriever(tpClient, null, true);
...
export function serializeNormalized(program, entities = {}, options = {}) {
 ...
 */
export function serializePrediction(prediction, sentence, entities, forTarget, options) {
    if (forTarget === 'user') {
