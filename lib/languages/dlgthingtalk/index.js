...
module.exports = {
    async parse(code, entities, options) {
        ...
        if (!policy.DIALOGUE_ACTS.has(state.dialogueAct))
            throw new Error(`${state.dialogueAct} is not a valid dialogue act for policy @${state.policy}`);
        state.delegate = policy;
        return state;
    },
