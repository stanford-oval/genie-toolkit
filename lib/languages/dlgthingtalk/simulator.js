...
class ResultGenerator {
    ...
    generate(schema, params) {
        ...
        for (let arg of schema.iterateArguments()) {
            ...
            if (arg.direction !== Ast.ArgDirection.OUT)
                continue;
            result[arg.name] = this._generateValue(arg.type, arg.name !== 'id');
        }
        return result;
    }
    ...
    _generateValue(type, repeatable = true) {
        ...
        if (type.isNumber)
            return this._generateNumber(`NUMBER`, repeatable);
        if (type.isMeasure)
            return this._generateNumber(`MEASURE_` + type.unit, repeatable);
        if (type.isCurrency)
...
class SimulationExecEnvironment extends ThingTalk.ExecEnvironment {
    ...
    async _doInvokeQuery(kind, fname, params) {
        ...
        const numResults = schema.is_list ? randint(50, 100, this._rng) : 1;
        const outputType = kind + ':' + fname;

        let results = [];
        for (let i = 0; i < numResults; i++)
            results.push([outputType, this.generator.generate(schema, params)]);

        if (schema.hasArgument('id')) {
            results.sort(([ot1, r1], [ot2, r2]) => {
                if (r1.id < r2.id)
                    return -1;
                if (r2.id > r1.id)
                    return 1;
    ...
    async invokeQuery(kind, attrs, fname, params) {
        ...
        const list = await this._doInvokeQuery(kind, fname, params);
