...
const { getBestEntityMatch } = require('./entity_utils');
...
class ThingTalkSimulatorState {
    ...
    async simulate(stmt, compiled) {
        ...
        this._execEnv.output = async (outputType, outputValue) => {
            ...
            if (mapped.id && mapped.id.isEntity && mapped.id.display) {
                const id = mapped.id;
                ...
                if (this._previousIdEntities.has(idType))
                    this._previousIdEntities.get(idType).push(idEntity);
                else
                    this._previousIdEntities.set(idType, [idEntity]);
            }
