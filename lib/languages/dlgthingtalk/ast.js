...
module.exports.DialogueState = class DialogueState {
    ...
    optimize() {
        ...
        this.stack = this.stack.map((prog) => prog.optimize()).filter((prog) => prog !== null);
        return this;
    }
    ...
    prettyprint() {
        ...
        for (let prog of this.history)
            code += prog.prettyprint().trim() + '\n';
        for (let prog of this.stack)
            code += prog.prettyprint().trim() + '\n';
        return code;
    }
    ...
    serialize(sentence) {
        ...
        for (let i = 0; i < this.stack.length; i++) {
            ...
            const prog = this.stack[i];
            code.push(...this._serializeProgram(prog, isLastProgram ? sentence : '', entities, isLastProgram));
            if (!isLastProgram)
                code.push(';');
        }
        code.push('}');
