...
module.exports = class SlotFillingDialog extends Dialog {
    ...
    _askFeed(question) {
        ...
        if (!messaging.isAvailable) {
            this.reply("Messaging is not available, cannot choose a feed.");
            return this.switchToDefault();
        }

        return Helpers.getFeedList(this.manager.messaging).then((feeds) => {
            this._aux = [];
    ...
    handlePicture(url) {
        if (this._resolving !== null &&
            this.expecting === ValueCategory.Picture) {
            this.values[this._resolving] = Ast.Value.Picture(url);
            this._resolving = null;
