...
function getCompatKey(permissionRule) {
    ...
    if (permissionRule.trigger.isStar)
        ...
        key = permissionRule.trigger.kind + ':' + permissionRule.trigger.channel;
    key += '+';
...
module.exports = class PermissionManager {
    ...
    start() {
        return this._db.getAll().then((rows) => Q.all(rows.map((row) => {
    ...
    getAllPermissions() {
        ...
        for (let [uniqueId, rule] of this._permissiondb) {
            ...
            ret.push({
                uniqueId: uniqueId,
                rule: rule,
                code: ThingTalk.Ast.prettyprintPermissionRule(rule),
    ...
    addPermission(permissionRule, description, extra = {}) {
        ...
        let code = ThingTalk.Ast.prettyprintPermissionRule(permissionRule);
    ...
    _cleanProgram(program) {
        function cleanValue(value) {
            if (value.isEntity && (value.type === 'tt:contact' || value.type === 'tt:contact_group'))
                value.display = null;
        }
        function cleanFilter(filter) {
            if (filter.isTrue || filter.isFalse)
                return;
            if (filter.isAtom) {
                cleanValue(filter.filter.value);
                return;
            }
            if (filter.isExternal) {
                cleanInvocation(filter);
                return;
            }
            if (filter.isNot) {
                cleanFilter(filter.expr);
                return;
            }

            filter.operands.forEach(cleanFilter);
        }

        function cleanInvocation(inv) {
            for (let in_param of inv.in_params)
                cleanValue(in_param.value);
            cleanFilter(inv.filter);
        }

        for (let rule of program.rules) {
            if (rule.trigger)
                cleanInvocation(rule.trigger);
            for (let query of rule.queries)
                cleanInvocation(query);
            for (let action of rule.actions)
                cleanInvocation(action);
        }
    }

    installProgram(principal, identity, program, uniqueId) {
        if (program.principal !== null)
            return Q.reject(throwCode('EPERM', 'Permission denied'));
        ...
        return this._isAllowedProgram(principal, identity, program).then((newProgram) => {
            ...
            return conversation.runProgram(newProgram, uniqueId);
        }).then(() => undefined);
    }
}
