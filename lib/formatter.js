...
module.exports = class Formatter extends ThingTalk.Formatter {
    ...
    _getFormatMetadata(outputType) {
        ...
        if (kind === 'com.nest' && channel === 'current_event') {
    ...
    formatForType(outputType, outputValue, hint) {
        ...
        if (outputType === null)
            return Object.keys(outputValue).map((key) => `${key}: ${outputValue[key]}`);

        return this._getFormatMetadata(outputType).then((metadata) => {
            if (metadata && metadata.formatted && metadata.formatted.length)
                return this.format(metadata.formatted, outputValue, hint);
