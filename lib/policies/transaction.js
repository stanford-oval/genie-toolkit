    ...
    async reply(dlg, state) {
        ...
        case 'sys_confirm_action':
            ...
            await dlg.replyInterp(dlg._("Ok, so you want me to ${confirmation}. Is that right?"), {
                confirmation: Describe.describeProgram(new Ast.Program(null, [], [], [nextItem.stmt]))
            });
            return;
        ...
        case 'sys_empty_search':
            await dlg.reply(dlg._("I'm sorry, I cannot find any result like that."));
            return;
        ...
        case 'sys_recommend_two':
            ...
            for (let result of (questionItem || currentItem).results.results) {
                console.log(result);
                await replyFormattedResult(dlg, result.raw);
            }
            break;
