...
module.exports = class ParameterReplacer {
    ...
    async _computeReplaceableParameters(context, code) {
        ...
        for (let i = 0; i < code.length; i++) {
            ...
            if (/^[A-Z]/.test(token)) {
                ...
                const escaped = token.replace(/[:._]/g, (match) => {
                    if (match === '_')
                        return '__';
        ...
        // now undo the replacement with SLOT_*
        for (let i = 0; i < code.length; i++) {
            const token = code[i];
            if (token.startsWith('SLOT_'))
                code[i] = entities[token].token;
        }
    ...
    async process(example) {
        ...
        const parameters = await this._computeReplaceableParameters(example.context ? example.context.split(' '): [], program);
