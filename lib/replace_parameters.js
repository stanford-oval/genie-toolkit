...
function isReplaceToken(tok) {
    return /^(?:GENERIC_ENTITY_|LOCATION_|PHONE_NUMBER|NUMBER_|QUOTED_STRING_|HASHTAG_|USERNAME_)/.test(tok);
}
...
module.exports = class ParameterReplacer {
    ...
    _getEntityListKey(entityType) {
        switch (entityType) {
            case 'tt:username': case 'tt:contact': case 'tt:email_address':
                return ['string', 'tt:person_first_name'];
    ...
    async _sampleParam(slot) {
        ...
        while (attempts > 0) {
            ...
            if (!this._paramLangPack.isGoodSentence(sampled) && (this._replaceNumbers && this._paramLangPack.isGoodNumber(sampled))) {
                attempts -= 1;
                continue;
            }
