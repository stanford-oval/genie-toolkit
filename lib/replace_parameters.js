...
module.exports = class ParameterReplacer {
    ...
    _transformValue(sentenceValue, programValue, arg) {
        ...
        // FIXME move to langpack for multi-language

        if (programValue.startsWith('the ') && programValue.length > 4) {
            // remove "the" from the program value if we have it
            programValue = programValue.substring(4);
            if (coin(0.5, this._rng))
                sentenceValue = sentenceValue.substring(4);
        } else if (arg.name === 'id' && coin(0.5, this._rng)) {
            // else add it to the sentence value if the argument is an entity
            assert(!sentenceValue.startsWith('the '));
            sentenceValue = 'the ' + sentenceValue;
        }
