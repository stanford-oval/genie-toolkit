...
module.exports = class ParameterReplacer {
    ...
    async _sampleParam(slot, replacedValuesSet) {
        ...
        // try to get argument-specific entity data first
        let valueList = await this._loader.get([valueListKey[0], valueListKey[1] + '_' + (slot.arg||slot._arg).name]);
        if (valueList.size === 0)
            valueList = await this._loader.get(valueListKey);
        if (valueList.size === 0) {
