...
module.exports = class ParameterReplacer {
    ...
    async _sampleParam(slot, replacements) {
        ...
        if (valueList.size === 0) {
            console.log(`Value list is empty: missing required parameter list ${valueListKey[1]}`);
            if (this._debug) {
                console.log(`Falling back to ${fallbackKey}`);
                if (!this._warned.has('novalue:' + slot.tag + ':' + slot.type)) {
                    console.error(`Found no values forclean-parameters ${slot.tag}:${slot.type}`);
        ...
        while (attempts > 0) {
            ...
            if (this._paramLangPack.isGoodNumber(sampled) && this._replaceNumbers && !Array.from(replacements.values()).includes(sampled))
                return sampled;
            if (this._cleanParameters && /[,?!.]/.test(sampled)) {
                attempts -= 1;
                continue;
            }
        ...
        console.log(`Could not replace even after ${attempts} attempts for slot ${slot}`);
        return null;
    }
    ...
    async _replaceTokensInSentence(id, sentence, parameters, replacements) {
        ...
        for (let token of sentence) {
            if (replacements.has(token)) {
                ...
                const replace = await this._sampleParam(parameters.get(token), replacements);
                if (!replace) {
                    output.push(token);
                } else {
