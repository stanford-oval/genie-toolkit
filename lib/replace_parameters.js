...
module.exports = class ParameterReplacer {
    ...
    async _replaceTokensInSentence(sentence, parameters, replacements) {
        let output = [];
    ...
    _outputReplacedTokenInProgram(token, replacement, output) {
    ...
    async _replaceTokensInContext(program, parameters, replacements) {
        let output = [];
        for (let token of program) {
            if (replacements.has(token)) {
                this._outputReplacedTokenInProgram(token, replacements.get(token), output);
                ...
                if (!replace) {
                    ...
                    replacements.set(token, replace);
                    this._outputReplacedTokenInProgram(token, replace, output);
                }
            } else {
    ...
    async _computeReplaceableParametersForProgram(parameters, code) {
    ...
    async _computeReplaceableParameters(context, sentence, code) {
        ...
        await this._computeReplaceableParametersForProgram(parameters, context);
        await this._computeReplaceableParametersForProgram(parameters, code);
        ...
        for (let i = 0; i < this._blowupFactor(example, parameters); i++) {
            promises.push((async () => {
                ...
                try {
                    let new_context;
                    if (example.context)
                        new_context = (await this._replaceTokensInContext(example.context.split(' '), parameters, replacements)).join(' ');
                    else
                        new_context = undefined;
                    const new_sentence = (await this._replaceTokensInSentence(sentence, parameters, replacements)).join(' ');
