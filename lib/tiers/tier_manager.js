...
const ALL_TIERS = [Tier.PHONE, Tier.SERVER, Tier.CLOUD, Tier.DESKTOP];
...
module.exports = class TierManager extends events.EventEmitter {
    ...
    _backoffTimer(tier) {
        let backoff = this._tierBackoffs[tier];
    ...
    _tryOpenOne(tier) {
        ...
        return socket.open().then((success) => {
            if (success) {
                let buffer = this._tierOutgoingBuffers[tier];
    ...
    _openAll() {
        ...
        for (let i = 0; i < ALL_TIERS.length; i++) {
            let p = this._tryOpenOne(ALL_TIERS[i]);
            if (p !== null)
                promises.push(p);
        }
    ...
    _openClient() {
        ...
        let toServer = () => {
            ...
            let serverAddress = prefs.get('server-address');
            if (serverAddress !== undefined) {
                return new tc.ClientConnection(serverAddress, this.ownTier,
        ...
        let toCloud = () => {
            ...
            if (cloudId !== undefined) {
                return new tc.ClientConnection(Config.THINGENGINE_URL + '/ws/' + cloudId,
                                               this.ownTier, Tier.CLOUD, authToken);
            } else {
                return null;
            }
        };
    ...
    _openServer() {
        ...
        let toPhone = function() {
            return new tc.ServerConnection(platform);
        };
        let toCloud = function() {
            let authToken = prefs.get('auth-token');
            let cloudId = prefs.get('cloud-id');
    ...
    _openCloud() {
        let authToken = this._platform.getAuthToken();
        if (authToken === undefined) {
            // Crash
            throw new Error('Cloud platform without a valid auth token!');
        }

        let cloudId = this._platform.getCloudId();
        if (cloudId === undefined) {
            // Crash
    ...
    closeOne(tier) {
        if (this._tierSockets[tier] !== null) {
            if (this._tierSockets[tier].isServer) {
                ...
                return this._tierSockets[tier].close().then(() => {
                    this._tierSockets[tier] = null;
                });
            }
        } else {
    ...
    reopenOne(tier) {
        return this.closeOne(tier).then(() => {
