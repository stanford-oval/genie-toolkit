...
const ProxyChannel = new Tp.ChannelClass({
    ...
    _init: function(proxyManager, targetTier, targetChannelId, localChannel, cachedArgs) {
        this.parent();
    ...
    formatEvent(event, filters, hint, formatter) {
        return this._localChannel.formatEvent(event, filters, hint, formatter);
    },
...
class ChannelStub {
    ...
    invokeQuery(filters) {
        return this._innerChannel.invokeQuery(filters);
    }

    formatEvent(event, filters) {
        return this._innerChannel.formatEvent(event, filters);
    }

    // called when whoever asked for us is requesting to push some data into
    // the channel
    sendEvent(data) {
        return this._innerChannel.sendEvent(data);
    }
...
module.exports = class ProxyManager {
    constructor(tierManager, channels, devices, messaging) {
        ...
        this._pipeManager = new PipeManager(tierManager, this);
    ...
    _sendInvokeChannelReply(targetTier, targetChannelId, requestId, error, value) {
        this._sendMessage(targetTier, {
            ...
            requestId: requestId,
            error: error,
            result: Protocol.values.marshal(value)
        });
    }
    ...
    getProxyChannel(targetChannelId, targetTier, device, localChannel, kind, mode, params) {
        var fullId = targetChannelId + '-' + targetTier;

        if (fullId in this._proxies)
            return this._proxies[fullId];

        var proxy = new ProxyChannel(this, targetTier, targetChannelId, localChannel, [device, kind, mode, params]);
    ...
    _requestChannel(fromTier, targetChannelId, device, kind, mode, params) {
        ...
        if (device === 'thingengine-pipe-system')
            channel = Q(this._pipeManager.getProxyNamedPipe(kind));
        else
            channel = this._channels.getChannel(device, kind, mode, params);
    ...
    _channelSourceData(fromTier, targetChannelId, data) {
        ...
        proxy.emitEvent(Protocol.params.unmarshal(this._messaging, data));
    }
    ...
    _handleChannelInvoke(fromTier, targetChannelId, fn, requestId, data1, data2) {
        ...
        this._stubs[fullId].then((stub) => {
            return stub[fn](Protocol.params.unmarshal(this._messaging, data1), Protocol.params.unmarshal(this._messaging, data2));
        }).then((result) => {
    ...
    _handleChannelInvokeResult(fromTier, targetChannelId, requestId, error, result) {
        var fullId = targetChannelId + '-' + fromTier + '-' + requestId;
