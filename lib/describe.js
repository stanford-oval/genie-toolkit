...
function describeTrigger(dlg, kind, channel, owner, schema, args, comparisons) {
    if (kind === '$remote') {
        // special case internal sending/receiving
        if (channel === 'send')
            return dlg._("send it to %s").format(owner);
        if (channel === 'receive')
            return dlg._("you receive something from %s").format(owner);
    }
