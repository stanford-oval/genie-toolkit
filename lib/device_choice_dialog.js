...
module.exports = class DeviceChoiceDialog extends Dialog {
    ...
    static chooseDevice(parent, obj) {
        ...
        if (parent.subdialog === null) {
            parent.push(new DeviceChoiceDialog(obj.kind, obj.principal));
            return parent.subdialog.continue().then((waiting) => {
                if (waiting) {
                    return waiting;
                } else {
                    if (parent.subdialog.principal) {
                        obj.principal = parent.subdialog.principal;
                    } else {
                        obj.id = parent.subdialog.device.uniqueId;
    ...
    _continueForeign() {
        if (this.owner.type === 'tt:contact') {
            this.principal = this.owner;
            return false;
        }
        ...
        this.contactSearch = Ast.InputParam('__principal', this.owner);
