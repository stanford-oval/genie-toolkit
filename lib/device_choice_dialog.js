...
module.exports = class DeviceChoiceDialog extends Dialog {
    ...
    _continueForeign() {
        if (this.owner.indexOf(':') >= 0) {
            var principal = this.owner;
            return this.manager.devices.getForeignDevice(principal, this.kind).then((device) => {
                this.device = device;
        ...
        if (this.contactSearch[0] !== null && !this.contactSearch[0].isVarRef) {
            var principal = this.contactSearch[0].value;
            return this.manager.devices.getForeignDevice(principal, this.kind).then((device) => {
                this.device = device;
                this.principal = principal;
                return false;
        ...
        this.contactSearch[0] = Ast.Value.VarRef('$contact(' + this.owner + ')');
