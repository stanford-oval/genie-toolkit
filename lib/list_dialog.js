...
module.exports = class ListDialog extends Dialog {
    ...
    _listCommands() {
        ...
        return this.manager.thingpedia.getExamplesByKey(typeArray.join(' '), true).then((examples) => {
            var added = new Set();
            examples.forEach((ex) => {
                if (added.has(ex.target_json))
                    return;
                added.add(ex.target_json);
                this.replyButton(tokenize(ex.utterance).map((t) => t.startsWith('$') ? '_____' : t).join(' '),
                    ex.target_json);
            });
