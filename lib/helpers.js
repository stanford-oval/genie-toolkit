...
module.exports = {
    ...
    sendRules(dlg, principalAndPrograms, app) {
        if (principalAndPrograms.length === 0)
            return;
        let identities = dlg.manager.messaging.getIdentities();
        let identity = this.findPrimaryIdentity(identities);
        if (!identity)
        ...
        for (let [principal, program] of principalAndPrograms) {
            ...
            let reconstructed = Describe.describeProgram(dlg, program);
            dlg.reply(dlg._("Sending rule to %s: %s").format(Describe.describeArg(dlg, principal), reconstructed));
            let json = SEMPRESyntax.toSEMPRE(program, true);
            dlg.manager.remote.installRuleRemote(principal.value, identity, json).catch((e) => {
                if (app) {
                    app.reportError(e);
