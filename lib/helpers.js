...
module.exports = {
    ...
    sendRules(dlg, principalAndRules, app) {
        if (principalAndRules.length === 0)
            return;
        var identities = dlg.manager.messaging.getIdentities();
        var identity = this.findPrimaryIdentity(identities);
        if (!identity)
            return Q.reject("Failed to find a suitable messaging identity");

        return Q.all(principalAndRules.map(([principal, rule]) => {
            return reconstruct(dlg, JSON.stringify(rule)).then((reconstructed) => {
                dlg.reply(dlg._("Sending rule to %s: %s").format(principal.display, reconstructed));
                dlg.manager.remote.installRuleRemote(principal.value, identity, rule)
                    .catch((e) => {
                        if (app) {
                            app.reportError(e);
