...
module.exports = {
    ...
    notify(dialog, appId, messages) {
        ...
        if (appId !== undefined)
            ...
            app = undefined;
        var icon = app ? (app.icon || null) : null;
    ...
    notifyError(dialog, appId, error) {
        var app;
        if (appId !== undefined)
            app = dialog.manager.apps.getApp(appId);
        else
        ...
        if (app !== undefined)
            dialog.reply(dialog._("%s had an error: %s.").format(app.name, errorMessage));
        else
            dialog.reply(dialog._("Sorry, that did not work: %s.").format(errorMessage));
        return true;
    }
}
