...
module.exports = {
    ...
    notify(dialog, appId, icon, messages) {
        var app;
        if (appId !== undefined)
        ...
        var notifyOne = (message) => {
            ...
            if (message.type === 'text') {
                dialog.reply(message.text, icon);
            } else if (message.type === 'picture') {
                dialog.replyPicture(message.url, icon);
            } else if (message.type === 'rdl') {
                dialog.replyRDL(message, icon);
            }
        }
        if (app !== undefined && app.isRunning &&
            ...
            dialog.reply(dialog._("Notification from %s: %s").format(app.name, messages), icon);
        } else {
            if (app !== undefined && app.isRunning)
                dialog.reply(dialog._("Notification from %s").format(app.name), icon);
    ...
    notifyError(dialog, appId, icon, error) {
        var app;
        if (appId !== undefined)
            app = dialog.manager.apps.getApp(appId);
        else
        ...
        if (typeof error === 'string')
            ...
            errorMessage = dialog._("Syntax error at %s line %d: %s").format(error.fileName, error.lineNumber, error.message);
        ...
        if (app !== undefined && app.isRunning)
            dialog.reply(dialog._("%s had an error: %s.").format(app.name, errorMessage), icon);
        else
            dialog.reply(dialog._("Sorry, that did not work: %s.").format(errorMessage), icon);
        return true;
    }
}
