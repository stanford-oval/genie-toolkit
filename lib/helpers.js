...
module.exports = {
        ...
        function doMake(from) {
            for (let argname in from) {
                ...
                if (type.isString)
                    ...
                    ops = ['=='];
                else if (type.isEntity && ['tt:picture', 'tt:url'].includes(type.type))
                    ops = [];
                else
                    ops = ['==', '!='];
                for (let op of ops)
    ...
    presentExampleList(dlg, examples, isLocal=true) {
        for (let ex of examples) {
            if (!isLocal)
                ex.utterance = ex.utterance.replace(/\b(my)\b/g, 'their').replace(/\b(me)\b/, 'them').replace(/\b(i)\b/g, 'they').replace(/\bnotify them\b/g, 'notify me');
