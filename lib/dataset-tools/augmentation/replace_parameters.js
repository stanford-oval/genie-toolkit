...
module.exports = class ParameterReplacer {
    ...
    _transformValue(sentenceValue, programValue, arg) {
        if (this._requotable) {
            if (arg && arg.name === 'id' && arg.type.isEntity && arg.type.type.endsWith(':Hotel')) {
                if (sentenceValue.startsWith('the '))
                    sentenceValue = sentenceValue.slice('the '.length).trim();
                if (sentenceValue.indexOf(',') !== -1)
                    sentenceValue = sentenceValue.slice(0, sentenceValue.indexOf(',')).trim();
                programValue = sentenceValue;
            }
    ...
    async _sampleParam(slot, replacedValuesSet) {
        ...
        while (attempts > 0) {
            ...
            if (sampled.includes('\n') || sampled.includes('/') || sampled.includes('\\') ){
                attempts -= 1;
                continue;
            }

            if (sampled === 'unspecified'){
                attempts -= 1;
                continue;
            }
