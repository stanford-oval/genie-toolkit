...
class DialogueEvaluatorStream extends Stream.Transform {
    ...
    async _valueToSlot(value) {
        ...
        if (value.isEntity) {
            ...
            return value.display.toLowerCase();
        }
        if (value.isBoolean)
        ...
        if (value.isLocation) {
            ...
            if (value.value.isAbsolute)
                return value.value.display.toLowerCase();
            // unresolved
        ...
        if (value.isString) {
            ...
            return this._tokenizer.tokenize(value.toJS()).rawTokens.join(' ');
        }
    ...
    async _checkTurn(id, turn, turnIndex) {
        ...
        const targetCode = (await this._target.serializePrediction(goldUserTarget, tokens, entities, 'user', {
            locale: this._locale
        })).join(' ');
