...
module.exports = class Dispatcher {
    constructor(manager) {
        ...
        this.formatter = new Formatter(manager);
        this.icon = null;
        this.expecting = null;
        this._choices = null;
    ...
    replySpecial(text, special) {
        let json = JSON.stringify({ code: ['bookkeeping', 'special', 'special:' + special], entities: {} });
    ...
    _handleGeneric(command) {
        ...
        if (this.expecting !== null &&
            ...
            if (this.expecting === ValueCategory.Command &&
                (command.isPrimitive || command.isHelp || command.isBack || command.isMore || command.isEmpty))
                return false;
            if (this.expecting === ValueCategory.Predicate &&
                (command.isPredicate || command.isFilter || command.isBack || command.isMore))
                return false;
            if (this.expecting === ValueCategory.PermissionResponse &&
                (command.isPredicate || command.isFilter || command.isPermissionRule || command.isMore || command.isYes || command.isMaybe || command.isBack))
                return false;
    ...
    dispatchNotify(appId, icon, outputType, outputValue, currentChannel) {
        let item = new QueueItem.Notification(appId, icon, outputType, outputValue, currentChannel);
        return this._pushQueueItem(item);
    }
    ...
    dispatchRunProgram(program, uniqueId) {
        let item = new QueueItem.RunProgram(program, uniqueId);
        return this._pushQueueItem(item);
    }
