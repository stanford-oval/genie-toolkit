...
module.exports = class Dispatcher {
    ...
    expect(expected) {
        ...
        if (expected === ValueCategory.YesNo)
            ...
            this.manager.sendAskSpecial('contact');
    ...
    ask(expected, question) {
        this.reply(question);
        return this.expect(expected).then((intent) => intent.value);
    }
    ...
    _handleGeneric(command) {
        ...
        if (this.expecting !== null &&
            ...
            if (this.expecting === ValueCategory.Command &&
                (command.isPrimitive ||
                 command.isHelp || command.isBack || command.isEmpty || command.isFilter))
                return false;
    ...
    _handleContextualHelp(command) {
        if (this._isInDefaultState())
            ...
            return this._doInject(Intent.Make).then(() => true);
        if (this.expecting !== null) {
    ...
    askForPermission(principal, identity, program) {
        let item = new QueueItem.PermissionRequest(principal, identity, program, resolve, reject);
        return this._pushQueueItem(item);
    }
    askQuestion(appId, icon, type, question) {
        let item = new QueueItem.Question(appId, icon, type, question, resolve, reject);
        return this._pushQueueItem(item);
    }
    ...
    handleRaw(raw) {
        let intent = Intent.Answer(ValueCategory.RawString, ThingTalk.Ast.Value.String(raw));
