...
class ContextStmt extends Statement {
    ...
    codegen(prefix = '') {
        return `${prefix}$grammar.declareContext(${this.name.codegen()}, ($) => ${this.code});\n`;
    }
}
...
class Constants extends Rule {
    constructor(token, typeCode, prefix, suffix) {
        super();
    ...
    codegen(nonTerminal, prefix = '') {
        return `${prefix}$grammar.addConstants(${nonTerminal.codegen()}, ${stringEscape(this.token)}, ${this.typeCode}, ${stringEscape(this.prefix)}, ${stringEscape(this.suffix)});\n`;
    }
}
