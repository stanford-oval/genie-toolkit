...
module.exports = class HelpDialog extends Dialog {
    ...
    _getCategoryList() {
        ...
        var titles = ['Media', 'Social Networks', 'Home', 'Communication', 'Health and Fitness', 'Services', 'Data Management'];
        for (var i = 0; i < 7; i++) {
    ...
    _getDeviceList(category) {
        ...
        return this.manager.thingpedia.getDeviceFactories(category).then((devices) => {
            ...
            if (device_list.length === 0) {
                this.reply(this._("Sorry, support of %s is still under developing ...").format(category));
                return this.switchToDefault();
            } else {
