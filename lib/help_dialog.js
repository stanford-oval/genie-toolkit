...
module.exports = class HelpDialog extends Dialog {
    ...
    _getCategoryList() {
        ...
        var titles = [this._('Media'), this._('Social Networks'), this._('Home'), this._('Communication'),
            this._('Health and Fitness'), this._('Services'), this._('Data Management')];
        for (var i = 0; i < 7; i++) {
            this._replyOneCategory(titles[i], categories[i]);
        }
        return this.switchToDefault();
    }
    ...
    _getDeviceList(category) {
        ...
        return this.manager.thingpedia.getDeviceFactories(category).then((devices) => {
            devices.forEach((device) => {
                console.log(device.global_name);
            ...
            if (device_list.length === 0) {
                this.reply(this._("Sorry, support of %s is still in development ...").format(category));
                return this.switchToDefault();
            } else {
                this.reply(this._("Here is the list of %s I support. Click on each of them to list their commands.").format(category));
