...
module.exports = class ChannelFactory {
    ...
    getChannel(device, kind, mode, params) {
        ...
        return Q.try(function() {
            ...
            var state;
            if (params !== undefined) {
                var mappedParams = params.map((p) => {
                    if (p === undefined)
                        return undefined;
                    if (p === null)
                        return null;
                    return ThingTalk.Ast.valueToJS(p);
                });
            } else {
                var mappedParams = undefined;
            }
            if (hasState) {
                state = new ChannelStateBinder(this._prefs);

                if (typeof factory === 'function') {
                    channel = new factory(this._engine, state, device, mappedParams);
                } else {
                    channel = factory.createChannel(this._engine, state, device, mappedParams);
                }
            } else {
                state = null;

                if (typeof factory === 'function') {
                    channel = new factory(this._engine, device, mappedParams);
                } else {
                    channel = factory.createChannel(this._engine, device, mappedParams);
                }
            }
