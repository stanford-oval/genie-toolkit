...
module.exports = class ChannelOpener extends ObjectSet.Base {
    ...
    _openOneChannel(device) {
        ...
        var promise;
        if (this._mode === 'r')
            promise = device.getTrigger(this._channelName, this.params);
        else if (this._mode === 'q')
            promise = device.getQuery(this._channelName, this.params);
        else if (this._mode === 'w')
            promise = device.getAction(this._channelName, this.params);
        else
