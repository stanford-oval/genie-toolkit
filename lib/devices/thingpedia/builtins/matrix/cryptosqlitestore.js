...
const MemoryCryptoStore = Matrix.MemoryCryptoStore;
...
module.exports = class CryptoSqliteStore {
    ...
    getOrAddOutgoingRoomKeyRequest(request) {
        return this._db.withTransaction((dbClient) => {
            return this._getOutgoingRoomKeyRequest(dbClient, request.requestBody).then((existing) => {
                if (existing) {
    ...
    getOutgoingRoomKeyRequestByState(wantedStates) {
        ...
        return this._db.withTransaction((dbClient) => {
            ...
            return JSON.parse(row[0].object);
        });
    }
