...
module.exports = class Messaging extends Tp.Messaging {
    ...
    _onTimeline(event, room, toStartOfTimeline, removed, data) {
        ...
        //console.log(direction, type, JSON.stringify(event.getContent()));
        console.log(direction, 'length ' + JSON.stringify(event.getWireContent()).length);

        let feed = this._feeds.get(room);
        if (feed)
            feed._newMessage(direction, obj);
        this.emit(direction, room.roomId, obj);
