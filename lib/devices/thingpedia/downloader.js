...
class BuiltinModule {
    ...
    getDeviceFactory() {
        ...
        var modulePath = path.resolve(path.dirname(module.filename), './builtins/', builtinId);
        this._loaded = require(modulePath);
        this._loaded.require = function(subpath) {
...
class CustomModule {
    ...
    _loadJsModule(id) {
        ...
        console.log('Loaded module ' + this.id + ' from cache');
        return module;
    }
...
module.exports = class ModuleDownloader {
    constructor(platform, client) {
        ...
        try {
            platform.makeVirtualSymlink(path.dirname(require.resolve('thingtalk')),
    ...
    _loadManifest(id, canUseCache) {
        ...
        return Q.try(() => {
            ...
            return this._client.getDeviceCode(id, 2).then((manifest) => {
                var stream = fs.createWriteStream(manifestTmpPath, { flags: 'wx', mode: 0o600 });
