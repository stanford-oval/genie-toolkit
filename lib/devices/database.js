...
module.exports = class DeviceDatabase extends ObjectSet.Base {
     constructor(platform, tierManager, factory, schemas) {
        ...
        this._subdeviceRemovedListener = this._notifySubdeviceAdded.bind(this);
    }
    ...
    async loadOneDevice(serializedDevice, addToDB) {
        ...
        try {
            const device = await this.factory.createDevice(serializedDevice.kind, serializedDevice);
    ...
    _notifySubdeviceAdded(subdevice) {
        ...
        const subsubdevices = device.queryInterface('subdevices');
    ...
    _startSubdevices(subdevices) {
        ...
        subdevices.on('object-removed', this._subdeviceAddedListener);
    ...
    _stopSubdevices(subdevices) {
        ...
        subdevices.removeListener('object-removed', this._subdeviceAddedListener);
    ...
    async _addDeviceInternal(device, uniqueId, addToDB) {
        ...
        if (device.constructor.metadata)
            this.schemas.injectManifest(device.kind, device.constructor.metadata);
    ...
    async updateDevicesOfKind(kind) {
        ...
        await this.factory.updateFactory(kind);
