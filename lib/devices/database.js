...
module.exports = class DeviceDatabase extends ObjectSet.Base {
     constructor(platform, tierManager, factory, schemas) {
        ...
        this._loadingRemoteDevices = new Map();
    }

    getForeignDevice(principal, kind) {
        var id = 'org.thingpedia.builtin.thingengine.remote-' + principal;
        if (this._devices.has(id))
            return Q(this._devices.get(id));

        if (this._loadingRemoteDevices.has(id))
            return this._loadingRemoteDevices.get(id);

        var promise = this.loadOneDevice({ kind: 'org.thingpedia.builtin.thingengine.remote', principal: principal }, true);
        this._loadingRemoteDevices.set(id, promise);
        return promise;
    }
