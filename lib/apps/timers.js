...
class AtTimer extends BaseTimer {
    ...
    toString() {
        return `[AtTimer [${this._times.join(', ')}], ${this._expiration_date}]`;
    }
    ...
    _nextTimeout() {
        ...
        if (this._expiration_date !== null && this._expiration_date < now) {
            console.log('AtTimer to the times ' + this._times.join(', ') + ': has hit expiration date of ' + this._expiration_date);
            return 86400000; // Will never return non-tomorrow interval if expiration date is hit
        }
        ...
        console.log('AtTimer to the times ' + this._times.join(', ') + ': polling again in ' + interval + ' ms');
        return interval;
    }
}
