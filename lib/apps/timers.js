...
class Timer extends BaseTimer {
    ...
    _nextTimeout(_now=null) {
        ...
        else if ((this._interval / this._frequency) < 10000) {
            throw new Error(`Timer with total interval ${this._interval} and frequency ${this._frequency} will have intervals of ${this._interval / this._frequency}. Minimum interval is 10 seconds.`);
        }
        // Special case if interval is 1 day
        else if (this._interval === DAY) {
            let timings = this._splitDay(this._frequency);
        ...
        // Special case if interval is 1 week
            ...
            if (base > now) {
                nextTiming = base;
            } else if (this._frequency === null || this._frequency === 1) {
                nextTiming = now + WEEK - ((now - base) % WEEK);
            } else if (this._frequency < 8) {
                // Hardcoded cases
                let days = this._splitWeek(this._frequency);
