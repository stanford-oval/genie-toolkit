...
module.exports = class MonitorRunner {
    constructor(env, devices, channel, params) {
        this._env = env;
    ...
    _onTriggerData(from, data) {
        ...
        // TODO check that data is not part of the saved stream already
        let now = Date.now();
        let outputType = from.device.kind + ':' + this._channel;
        for (let item of data) {
            item.__timestamp = now;
            this._queue.push({ done: false, value: [outputType, from.device, data] });
        }
    }
    ...
    _onDeviceAdded(device) {
        ...
        Promise.resolve().then(() => {
            ...
            console.error('Failed to initialize stream ' + uniqueId);
            this._env.reportError(e);
        });
    }
