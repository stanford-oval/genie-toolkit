    ...
    sendEvent(event, env) {
        Q.try(() => {
            if (event.length > 0) {
                ...
                // keep the channel open while we format
                return currentChannel.open().then(() => {
                    if (env.queryInput !== null)
                        return currentChannel.formatEvent(env.queryValue, env.queryInput);
                    else
                        return currentChannel.formatEvent(env.triggerValue);
                }).tap(() => {
            ...
            this.engine.platform.getCapability('assistant').notify([this._app.uniqueId, icon, formatted]);
        }).catch((e) => {
...
module.exports = class AppExecutor extends events.EventEmitter {
    ...
    reportError(error) {
        ...
        if (platform.hasCapability('assistant'))
            return platform.getCapability('assistant').notifyError([this.uniqueId, this.icon, error]);
    }
