...
module.exports = class AppExecutor extends events.EventEmitter {
    ...
    runCommands() {
        ...
        return Q.all(this.commands.map((c) => c.waitFinished())).then(() => {
            return this._mainOutput.done();
        });
    }
    ...
    open() {
        ...
        for (let [functions, code] of this.compiler.rules) {
            let hasTrigger = functions.length > 0 && functions[0].type === 'trigger';
            if (hasTrigger)
                this.rules.push(new RuleExecutor(this.engine, this, functions, code, this.conversationOutput));
            else
                this.commands.push(new RuleExecutor(this.engine, this, functions, code, this.mainOutput));
        }
