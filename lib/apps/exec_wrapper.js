...
module.exports = class ExecWrapper extends ExecEnvironment {
    ...
    getEventType(channel) {
        if (channel === null)
            return null;

        // NOTE: we used to apply masquerading for @remote.receive, to pass the right
        // output type to "notify;", but this confuses the compiler greatly, so now
        // we pass @remote:receive as the output type and the let the Almond layer
        // deal with it
        return new ThingTalk.Entity(channel.device.kind + ':' + channel.name, null);
    }
    ...
    _save(channel, input, output) {
        let table = this._genTableName(channel, input);
        return this.engine.memory.hasTable(table).then((exist) => {
            if (exist)
                return this.engine.memory.insertOne(table, output);
            return this.engine.schemas.getMetaByChannel(channel).then((meta) => {
                return this.engine.memory.insertOne(table, output, meta.args, meta.schema);
            });
        }).catch((e) => {
            console.log('Failed to save to almond memory: ' + e);
            console.log(e.stack);
        });
    }

    _genTableName(channel, input) {
        //TODO: better way to generate table name
        return channel.device.globalName + '.' + channel.name;
    }
            ...
            return Q.all(channels.map((c) => safeInvoke(c.invokeQuery(params)))).then((results) => {
                ...
                results.forEach((result, i) => {
                    ...
                    for (let element of result) {
                        ...
                        this._save(channel, params, element);
                    }
                });
    ...
    invokeMemoryQuery(params, aggregation) {
        //TODO: parse filters in memory instead of in thingtalk
        let table = params[0];
        return this.engine.memory.get(params[0], null, [], null, aggregation).then((results) => {
            let list = [];
            results.forEach((result) => {
                let result_list = Object.keys(result).map((k) => result[k]).slice(2);
                list.push([null, null, result_list]);
            })
            return list;
        });
    }
