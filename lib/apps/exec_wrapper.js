...
module.exports = class ExecWrapper extends ExecEnvironment {
    ...
    _save(channel, input, output) {
        let time = new Date();
        this.engine.schemas.getMetaByChannel(channel).then((schema) => {
            let device = channel.device.globalName;
            let random_id = Math.random().toString(36).slice(2,18);
            let record = {
                '_id': [time.getTime().toString(), random_id].join('-'),
                'timestamp': time,
