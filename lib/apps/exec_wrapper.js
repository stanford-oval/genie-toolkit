...
module.exports = class ExecWrapper extends ExecEnvironment {
    ...
    output(outputType, outputValues, currentChannel) {
        ...
        if (conversation)
            return conversation.notify(this.app.uniqueId, this.icon, outputType, outputValues, currentChannel);
        else
            return assistant.notifyAll(this.app.uniqueId, this.icon, outputType, outputValues, currentChannel);
    }
    ...
    say(message) {
        ...
        var conversation = this.getConversation();
        if (conversation)
            return conversation.notify(this.app.uniqueId, this.icon, null, [message]);
        else
            return assistant.notifyAll(this.app.uniqueId, this.icon, null, [message]);
    }
