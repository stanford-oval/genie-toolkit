...
module.exports = class ExecWrapper extends ExecEnvironment {
    ...
    invokeMemoryQuery(table, version, aggregation) {
        ...
        let cols = aggregation ? aggregation.cols : [];
        return this.engine.memory.get(table, version, cols, null, aggregation).then((results) => {
            let list = [];
            results.forEach((result) => {
                let result_list = Object.keys(result).map((k) => result[k]);
