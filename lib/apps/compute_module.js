...
module.exports = class ComputeModule {
    ...
    getTrigger(id) {
        if (id in this._module.events) {
            var ch = this.engine.pipes.getLocalSourceNamedPipe(this.uniqueId + '-' + id);
            return ch.open().then(function() {
                return ch;
            })
        } else {
    ...
    _getKeyword(name, decl) {
        return this.engine.keywords.getOpenedKeyword(this.app.uniqueId, name);
    }
    ...
    _startEventPipes() {
        ...
        return Q.all(eventnames.map(function(name) {
            this._eventPipes[name] = this.engine.getLocalSinkNamedPipe(this.uniqueId + '-' + name);
            return this._eventPipes[name].open();
        }, this));
    }
