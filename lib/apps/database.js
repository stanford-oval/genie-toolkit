...
module.exports = class AppDatabase extends events.EventEmitter {
    ...
    loadOneApp(code, state, uniqueId, tier, name, description, addToDB) {
        return Q.try(() => {
            ...
            this._ensureAppUniqueId(app, uniqueId);
            if (addToDB)
                return app.runCommands();
        }).tap((app) => {
            // only save into db apps that actually have some rules
            if (app.rules.length > 0)
                return this._addAppInternal(app, addToDB);
        }).catch(function(e) {
    ...
    _ensureAppUniqueId(app, uniqueId) {
