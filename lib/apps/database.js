...
module.exports = class AppDatabase extends events.EventEmitter {
    ...
    async createApp(program, options = {}) {
        for (let [, slot] of program.iterateSlots()) {
            ...
            if ((value.isUndefined && value.local) || !value.isConcrete())
        ...
        const appMeta = {
            $icon: options.icon || Helpers.getProgramIcon(program),
            $conversation: options.conversation||null
        };
        for (let key in options) {
            if (['uniqueId', 'name', 'description', 'icon', 'conversation'].indexOf(key) >= 0)
                continue;
            appMeta[key] = options[key];
        }
        ...
        if (program.principal !== null) {
            ...
            const identities = this._engine.messaging.getIdentities();
            ...
            if (ownprograms.length > 0) {
                for (let prog of ownprograms)
                    app = await this._startNewLocalApp(prog, uniqueId, appMeta, name, description);
            }
            ...
            return this._startNewLocalApp(program, uniqueId, appMeta, name, description);
        }
    }
