...
module.exports = class ActionExecutor {
    ...
    _addToQueue(env) {
        if (this._queue)
            this._queue = this._queue.then(() => this._runOne(env));
        else
            this._queue = this._runOne(env);
    }

    _runOne(env) {
        // keep our parent alive for the duration of the call

        // re null handling: a null queryValue is used to signify the
        // end of a query; it is passed to remote devices (which send it
        // over as a control message) but otherwise dropped, because
        // in general thingpedia actions choke if the event is null
        return this._parent.open().then(() => {
            if (env.queryInput !== null && env.queryValue === null)
                return null;
            return Q.all(this.output.produce.map((p) => p(env)));
        }).then((value) => {
            if (this._invokedApp) {
                if (value === null)
                    return;
                ...
                if (value === null) {
                    if (this.selector.isRemote) {
                        // HACK HACK HACK
                        // we know that the first argument to a $remote.send is
                        // a constant, so we can always evaluate it
                        var principal = this.output.produce[0](env);
                        var uuid = this.output.produce[1](env);
                        var channels = this.selector.values();
                        channels.forEach((channel) => {
                            Q.try(function() {
                                return channel.sendEvent(value, env, principal, uuid);
                            }).catch((e) => {
