...
module.exports = class ChannelOpener extends ObjectSet.Base {
    ...
    _isRemoteSelector(selector) {
        if (!selector.isAttributes)
            return false;

        return selector.attributes.some((attr) => {
            return attr.name === 'type' && attr.value === '$remote';
        });
    }

    _getRemotePrincipal(selector) {
        var principal;
        selector.attributes.forEach((attr) => {
            if (attr.name === 'principal')
    ...
    _normalizeSelector(selector, channelName) {
        if (this._isRemoteSelector(selector)) {
            var principal = this._getRemotePrincipal(selector);
            this._view = new ObjectSet.Simple();
            var remotedev = this.engine.devices.getForeignDevice(principal);
            this._remoteDevices.push(remotedev);
            this._view.addOne(remotedev);
        } else if (selector.isBuiltin) {
            this._view = new ObjectSet.Simple();
            this._view.addOne(this.app);
        } else if (this.app && selector.isGlobalName && selector.name === 'builtin' && this._channelName === 'timer') {
            // special handling @builtin.timer to have separate timers for separate rules, instead of
            // a global timer
            this._view = new ObjectSet.Simple();
            this._view.addOne(this.app);
        } else if (this.app && selector.isGlobalName && selector.name === 'builtin' && this._channelName === 'notify') {
            // special handling @builtin.notify (aka "say") to use the right conversation
            this._view = new ObjectSet.Simple();
            this._view.addOne(this.app);
        } else if (selector.isGlobalName && this.app && this.app.hasComputeModule(selector.name)) {
            this._view = new ObjectSet.Simple();
