...
module.exports = class CodegenDialog extends Dialog {
    ...
    describe() {
        var description = Describe.describeAction(this, this);
        ...
        return Q.try(() => {
            ...
            return Q.all(sendrules.map(([principal, rule]) => {
                return reconstruct(this, JSON.stringify(rule)).then((reconstructed) => {
                    this.reply(this._("Sending rule to %s: %s").format(principal.display, reconstructed));
                    this.manager.remote.installRuleRemote(principal.value, rule)
                        .catch((e) => {
                            if (app) {
                                app.reportError(e);
