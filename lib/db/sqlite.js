...
function initializeDB(filename, key) {
...
function migrateTo1(filename, key) {
...
function migrateDB(platform) {
    ...
    if (version < 1)
        return migrateTo1(filename, key);
    return Q();
}
...
module.exports = {
    ...
    ensureSchema: function(platform) {
        if (!fs.existsSync(platform.getSqliteDB())) {
            return initializeDB(platform.getSqliteDB(), platform.getSqliteKey());
        } else {
