...
async function slotFillArray(dlg, elementType, question) {
    ...
    do {
        ...
        await dlg.replyInterp(dlg._("You chose ${array}."), {
            array: values.map((v) => Describe.describeArg(dlg.manager.gettext, v))
        });
...
async function slotFillCustomRaw(dlg, slotType, question) {
    if (slotType.isArray)
        ...
        return new Ast.Value.Boolean(await dlg.ask(ValueCategory.YesNo, question));
        ...
        return new Ast.Value.Enum(slotType.entries[await dlg.askChoices(question, slotType.entries)]);
    else
...
async function concretizeValue(dlg, slot, hints = {}) {
    ...
    if (value.isEntity && value.type === 'tt:contact') {
        ...
        const resolved = await lookupEntity(dlg, value.type, value.display, hints.idEntities || []);
        if (resolved === null)
            return false;
        ...
        if (options.length > 0) {
            let choices = options.map((o) => {
                if (o.value.isEvent) {
                    ...
                    return dlg.interpolate(dlg._("Use the ${argcanonical} from ${device}"), {
                        argcanonical: o.argcanonical,
                        device: Helpers.cleanKind(o.kind)
                    });
                }
            });
