...
class Session {
    ...
    sendUtterance(utterance) {
        // HACK HACK HACK
        // recognize and eliminate "hey sabrina" from the utterance

        utterance = utterance.replace(/hey\s+sabrina/i, '');
        // if "hey sabrina" was everything there was, return "hello"
        if (utterance.trim().length === 0)
            return Q(JSON.stringify({ special: 'tt:root.special.hello' }));

        return httpRequest(this._baseUrl + '/query?locale=' + this._locale + '&q=' + encodeURIComponent(utterance),
...
module.exports = class SempreClient {
    ...
    openSession(locale) {
        return new Session(this._baseUrl, locale || 'en_US');
    }
}
