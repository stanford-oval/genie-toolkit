...
module.exports = async function setupDialog(dlg, intent, confident) {
    ...
    const icon = Helpers.getProgramIcon(intent.program);
    dlg.icon = icon;

    let description = Describe.describeProgram(dlg.manager.gettext, intent.program);

    if (!confident) {
        await dlg.setContext(intent.program);
    ...
    let hasSlots = false;
    for (let [schema, slot] of intent.program.iterateSlots()) {
        if (!(slot instanceof ThingTalk.Ast.InputParam) && !(slot instanceof ThingTalk.Ast.BooleanExpression))
            continue;
    ...
    if (echo)
        dlg.reply(dlg._("Ok, so I'm going to %s.").format(description));
