...
function confirm(dlg, description, source) {
    if (source)
        return dlg.ask(ValueCategory.YesNo, dlg._("Okay, so you want me to %s (as asked by %s). Is that right?").format(description, source));
    else
        return dlg.ask(ValueCategory.YesNo, dlg._("Okay, so you want me to %s. Is that right?").format(description));
}
...
module.exports = async function ruleDialog(dlg, intent, confident, uniqueId, sourceIdentity) {
    ...
    if (echo) {
        if (source)
            ...
            await dlg.reply(dlg._("Okay, I'm going to %s.").format(description));
    }
