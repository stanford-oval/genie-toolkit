...
const SLOT_FILLING_IMPLEMENTED = false;
...
module.exports = function* ruleDialog(dlg, intent, skipConfirmation, uniqueId) {
    ...
    let autoConfirm = true, hasTrigger = true;
    dlg.icon = null;
    if (SLOT_FILLING_IMPLEMENTED) {
        let ok;
        ({ ok, autoConfirm, hasTrigger } = yield* completeProgram(dlg, program, skipConfirmation));
        if (!ok)
            return;
    }
