...
const { slotFillPrimitive, slotFillProgram } = require('./slot_filling');
...
function* completeProgram(dlg, program, skipConfirmation) {
    ...
    let ok = yield* slotFillProgram(dlg, program, scope);
    if (!ok)
        return { ok };
...
module.exports = function* ruleDialog(dlg, intent, skipConfirmation, uniqueId) {
    ...
    console.log('About to execute program', Ast.prettyprint(program));
