...
module.exports = async function permissionRule(dlg, intent, confident) {
    ...
    if (!confident) {
        ...
        let confirmation = await dlg.ask(ValueCategory.YesNo, dlg._("Okay, so %s. Is that right?").format(description));
        if (!confirmation) {
            dlg.reset();
            return;
        }
    ...
    if (echo)
        await dlg.reply(dlg._("Okay, I'm going to set: %s.").format(description));
