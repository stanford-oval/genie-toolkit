...
async function checkPermissionRule(dlg, principal, program, permissionRule) {
    await dlg.reply(dlg._("Okay, I'll remember that."));
...
async function addFilter(dlg, rule) {
    ...
    for (;;) {
        ...
        */
        ...
        prim.filter = new Ast.BooleanExpression.And(null, [originalFilter, predicate]).optimize();
...
module.exports = async function permissionGrant(dlg, program, principal, identity) {
    ...
    const sourceContact = new Ast.Value.Entity(identity, 'tt:contact', null);
    ...
    try {
        if (permissionRule) {
            ...
            await dlg.replyInterp(dlg._("${principal} would like to ${command}."), {
            ...
            for (;;) {
                ...
                if (hasParam) {
                    ...
                    await dlg.replyButton(dlg.interpolate(
                        dlg._("Always from ${principal} (no restrictions)"),
                        { principal: contactName }), {
                        program: emptyPermissionRule.prettyprint(),
                    });
                    await dlg.replyButton(dlg.interpolate(
                        dlg._("Always from ${principal} (this exact request)"),
                        { principal: contactName }), {
                ...
                if (answer.isPermissionRule) {
                    ...
                    ok = await dlg.ask(ValueCategory.YesNo, dlg._("Okay, so ${permission}. Is that correct?"), {
                ...
                if (answer.isMaybe) {
                    ...
                    let ok = await dlg.ask(ValueCategory.YesNo, dlg._("Okay, so ${permission}. Is that correct?"), {
                        permission: description
                    });
                    if (!ok)
                        continue;
            ...
            let ok = await dlg.ask(ValueCategory.YesNo, dlg._("${principal} would like to ${command}."), {
                principal: contactName,
                command: description
            });
            if (ok)
