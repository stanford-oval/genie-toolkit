...
function getDeviceHelp(dlg, category, name, page) {
    return dlg.manager.thingpedia.getExamplesByKinds([name], true).then((examples) => {
        ...
        dlg.reply(dlg._("Here's what I can do for you on %s.").format(this.name.replace(/_/g, ' ')));
...
function replyOneDevice(dlg, title, kind, category) {
    return dlg.replyButton(title, JSON.stringify({ command: { type: 'help', value: { id: 'tt:device.' + kind }, category: category }}));
}
...
module.exports = function* helpDialog(dlg) {
    let intent = dlg.nextIntent();
