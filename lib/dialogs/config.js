...
async function runFormConfiguration(dlg, factory) {
    ...
    await dlg.manager.devices.loadOneDevice(state, true);
...
module.exports = async function configDialog(dlg, kind) {
    ...
    if (!factory) {
        ...
        await dlg.manager.devices.loadOneDevice({ kind: factory.kind }, true);
        ...
        if (factory.choices.length > 0) {
            ...
            dlg.reply(dlg._("Sorry, I don't know how to configure %s.").format(Helpers.cleanKind(kind)));
        }
    } else if (factory.type === 'interactive') {
        const delegate = new DiscoveryDelegate(dlg, factory.category);

        await dlg.manager.devices.factory.runInteractiveConfiguration(factory.kind, delegate);
