...
function* getExamples(dlg, raw) {
    ...
    if (examples.length === 0)
        ...
        return yield* failWithOptions(examples);
}
...
function* failWithFallbacks(dlg, fallbacks, isTrain, rephrasing) {
    ...
    if (countCanonicals === 0) {
        ...
        let idx = dlg.askChoices(dlg._("Did you mean any of the following?"), choices.map(([json, text]) => text));
        if (idx === choices.length - 1) {
            if (isTrain) {
                ...
                setImmediate(() => {
                    dlg.manager.handleParsedCommand(train);
                });
                return null;
            }
        } else {
...
module.exports = function* fallback(dlg, intent) {
    ...
    if (learn) {
        ...
        dlg.manager.sempre.onlineLearn(raw, chosen);
