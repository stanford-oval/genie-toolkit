...
function makeProgram(trigger, query, action, executor=null) {
    let rule;
    if (trigger && query)
        rule = new Ast.Statement.Rule(new Ast.Stream.Join(trigger, query, [], null), action ? [action] : [ThingTalk.Generate.notifyAction()]);
    else if (trigger)
        rule = new Ast.Statement.Rule(trigger, action ? [action] : [ThingTalk.Generate.notifyAction()]);
    else
        rule = new Ast.Statement.Command(query, action ? [action] : [ThingTalk.Generate.notifyAction()]);
...
module.exports = function* makeDialog(dlg) {
    ...
    let hint = "Do you want to use your own account or others?";
    let choices = ["Use my own account", "Use others account"];
    let choice = yield dlg.askChoices(hint, choices);
    let isLocal = choice === 0;
    let user = null;

    if (!isLocal) {
        let username = yield dlg.ask(ValueCategory.RawString, "Whose account do you want to use?");
        user = yield* contactSearch(dlg, Type.Entity('tt:contact'), username.value);
        console.log(user)
    }
