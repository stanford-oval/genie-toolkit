...
function* fillOnePrimitive(dlg, primType) {
    ...
    while (true) {
        ...
        if (intent.isEmpty) {
            ...
            return [cleanCanonical(primType, schema.canonical), root, makeFilterCandidates(schema)];
        }
    }
}
...
module.exports = function* makeDialog(dlg) {
    ...
    while (true) {
        ...
        if (index === 3) {
            ...
            let filter = filterIntent.root.filter;
            ...
            if (filter.value === null) {
                ...
                case 'Measure':
                    ...
                    break;
                case 'Bool':
                    // YesNo questions are different than other questions, they return a
                    // boolean instead of an Ast.Value
            ...
            if (result === null)
                continue;
            let [canonical, root, filterCand] = result;
            utterances[index] = canonical;
            rule[primType] = root;
            filterCandidates[index] = filterCand;
    ...
    for (let idx = 0; idx < 3; idx++) {
        filters[idx].forEach((filter) => {
            ...
            rule[PRIM_TYPES[idx]].args.push(arg);
        });
    }

    let programIntent = yield Intent.parse({ rule: rule }, dlg.manager.schemas, null, null, null);
