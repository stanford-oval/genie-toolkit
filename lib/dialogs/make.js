...
const { slotFillCustom, concretizeValue } = require('./slot_filling');
...
async function getDeviceHelp(dlg, category, name, page, primType, isLocal=true, keywords=[]) {
    ...
    let examples;
    if (primType === 'all') {
        let triggers = allExamples.filter((ex) => ex.type === 'query' && ex.monitorable);
...
module.exports = async function makeDialog(dlg) {
    ...
    if (ENABLE_REMOTE_COMMANDS) {
        ...
        if (!isLocal) {
            ...
            let holder = { value: user };
            let ok = concretizeValue(dlg, holder, Type.Entity('tt:contact'));
            if (!ok)
                return;
            user = holder.value;
        }
