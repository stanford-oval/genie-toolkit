...
module.exports = async function makeDialog(dlg, startIntent) {
    ...
    if (rule.stream !== null && filters[0] && filters[0].length > 0)
        rule.stream = new Ast.Stream.Filter(rule.stream, new Ast.BooleanExpression.And(filters[0]), rule.stream.schema);
    if (rule.query !== null && filters[1] && filters[1].length > 0)
        rule.query = new Ast.Table.Filter(rule.query, new Ast.BooleanExpression.And(filters[1]), rule.query.schema);
