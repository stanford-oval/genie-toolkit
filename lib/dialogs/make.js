...
function getDeviceHelp(dlg, category, name, page, primType) {
    return dlg.manager.thingpedia.getExamplesByKinds([name], true)
        ...
        examples = examples.filter((ex) => ex.type === exType);
...
function describeFilter(dlg, _T, schema, filter, forButton) {
...
function describePredicate(dlg, _T, schema, expr) {
    return (function recursiveHelper(expr) {
        ...
        return describeFilter(dlg, _T, schema, expr);
    })(expr);
}
    ...
    while (true) {
        ...
        if (index === 3) {
            ...
            filterCandidates[index].forEach((filter) => {
                ...
                dlg.replyButton(Helpers.presentExample(dlg, describeFilter(dlg, _T, schema, filter, true)), obj);
            });
            dlg.replySpecial(dlg._("Back"), 'back');
