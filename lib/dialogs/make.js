...
function getDeviceHelp(dlg, category, name, page, primType, isLocal=true, keywords=[]) {
    return dlg.manager.thingpedia.getExamplesByKinds([name], true)
        ...
        .then((allExamples) => {
            ...
            let examples;
            if (primType === 'all') {
                let triggers = allExamples.filter((ex) => ex !== null && ex.type === 'table' && ex.monitorable);
                ...
                examples = allExamples.filter((ex) => ex !== null && ex.type === exType);
...
module.exports = function* makeDialog(dlg) {
    ...
    if (when_get_do) {
        ...
        while(true) {
            ...
            let choices = [ready? dlg._("%s").format(utterances[0] + filterDescription[0]) : dlg._("Pick a device to start")];
