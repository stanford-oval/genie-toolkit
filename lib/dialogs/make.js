...
const CATEGORIES = ['time', 'location', 'media', 'social-network', 'home', 'communication', 'service', 'data-management'];
...
function getDeviceList(dlg, category, primType, isLocal=true) {
    ...
    if (category === 'time') {
        let kind = 'org.thingpedia.builtin.thingengine.builtin';
        return getDeviceHelp(dlg, category, kind, 0, primType, isLocal, ['time', 'date']) ;
    ...
    return dlg.manager.thingpedia.getDeviceFactories(category).then((devices) => {
        ...
        device_list.forEach(([title, kind]) => {
            if (kind === 'com.nest')
                title = 'Nest Security Camera & Thermostat';
...
module.exports = function* makeDialog(dlg) {
    ...
    if (when_get_do) {
        while (true) {
            ...
            if (index === 3) {
                ...
                yield predicate.typecheck(schema, null, dlg.manager.schemas, {}, true);
                ...
                yield program.typecheck(dlg.manager.schemas, true);
        ...
        while(true) {
            ...
            if (ready) {
                ...
                hint = dlg._("Your command is: %s. You can add more filters or run your command if you are ready.")
                    .format(description);
                index = yield dlg.askChoices(hint, choices);
            } else {
                index = 0;
            }
            ...
            if (index === 1) {
                ...
                let prim;
                [primType, prim] = yield* fillOnePrimitive(dlg, 'all', isLocal);
                if (prim === undefined)
                    continue;
                if (prim === null) {
                    dlg.fail();
    ...
    if (program.principal)
        return yield* setupDialog(dlg, new Intent.Setup(program.optimize()));
    else
        return yield* ruleDialog(dlg, new Intent.Program(program.optimize()), true);
};