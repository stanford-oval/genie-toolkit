...
function getDeviceList(dlg, category, primType, isLocal=true) {
    ...
    // filters: example_id of all the examples about location
    if (category === 'location') {
        let kind = 'org.thingpedia.builtin.thingengine.phone';
        return getDeviceHelp(dlg, category, kind, 0, primType, isLocal, [1550300, 1550297, 1550295]) ;
    }
...
function getDeviceHelp(dlg, category, name, page, primType, isLocal=true, filters=[]) {
    return dlg.manager.thingpedia.getExamplesByKinds([name], true)
        ...
        examples = examples.filter((ex) => {
            if (filters.length !== 0) {
                console.log(filters, ex.target.example_id);
                if (!filters.includes(ex.target.example_id))
                    return false;
            }
