...
module.exports = function* slotFillPrimitive(dlg, prim, scope) {
    ...
    for (let slot of toConcretize) {
        ...
        if (value.isEntity && value.type === 'tt:contact_name')
            slot.value = yield* contactSearch(dlg, getType(slot), slot);
        if (value.isLocation && value.value.isRelative)
            slot.value = yield* resolveUserContext(dlg, slot, '$context.location.' + value.value.relativeTag);
        if (slot.value === null)
            return false;
    }
    ...
    for (let slot of toFill) {
        ...
        if (options.length > 0) {
            ...
            slot.value = options[choice.value].value;
            continue;
        }

        if (slotType.isEntity && slotType.type === 'tt:phone_number')
            slot.value = yield dlg.ask(ValueCategory.PhoneNumber, question);
        else if (slotType.isEntity && slotType.type === 'tt:email_address')
            slot.value = yield dlg.ask(ValueCategory.EmailAddress, question);
        else if (slotType.isEntity && slotType.type === 'tt:picture')
            slot.value = yield dlg.ask(ValueCategory.Picture, question);
        else if (slotType.isEntity && slotType.type === 'tt:contact')
            slot.value = yield dlg.ask(ValueCategory.PhoneNumber, question);
        else if (slotType.isString || slotType.isEntity)
            slot.value = yield dlg.ask(ValueCategory.RawString, question);
        else if (slotType.isMeasure)
            slot.value = yield dlg.ask(ValueCategory.Measure(slotType.unit), question);
        else if (slotType.isNumber)
            slot.value = yield dlg.ask(ValueCategory.Number, question);
        else if (slotType.isBoolean)
            slot.value = yield dlg.ask(ValueCategory.YesNo, question);
        else if (slotType.isDate)
            slot.value = yield dlg.ask(ValueCategory.Date, question);
        else if (slotType.isTime)
            slot.value = yield dlg.ask(ValueCategory.Time, question);
        else if (slotType.isLocation)
            slot.value = yield dlg.ask(ValueCategory.Location, question);
        else if (slotType.isEnum) {
            let entries = slotType.entries;
            let options = entries.map((e) => Ast.Value.Enum(e));
            let choice = yield dlg.askChoices(question, entries);
            slot.value = options[choice];
        } else {
