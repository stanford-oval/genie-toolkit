...
function* slotFillPrimitive(dlg, prim, scope) {
    ...
    for (let in_param of prim.in_params) {
        let ok = true;
        if (in_param.value.isUndefined && in_param.value.local)
            ok = yield* slotFillSingle(dlg, prim.schema, in_param);
        else if (!in_param.value.isConcrete())
            ok = yield* concretizeSingleSlot(dlg, prim.schema, in_param);
        if (!ok)
            return false;
    }
...
function* slotFillProgram(dlg, program, scope) {
    for (let [schema, slot] of Generate.iterateSlots(program)) {
        let ok = true;
        if (slot.value.isUndefined && slot.value.local)
            ok = yield* slotFillSingle(dlg, schema, slot, scope);
        else if (!slot.value.isConcrete())
            ok = yield* concretizeSingleSlot(dlg, schema, slot, scope);
        if (!ok)
            return false;
    }
