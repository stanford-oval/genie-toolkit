...
function* doOneSlot(dlg, schema, slot, prim, scope, setupProgram) {
...
function* slotFillProgram(dlg, program) {
    if (program instanceof Ast.Program) {
        ...
        for (let [schema, slot, prim, scope] of Generate.iterateSlots(program)) {
            let ok = yield* doOneSlot(dlg, schema, slot, prim, scope, setupProgram);
            if (!ok)
                return false;
        }
    } else {
        ...
        for (let [schema, slot, prim, scope] of Generate.iterateSlotsFilter(null, program.principal, null, {})) {
            if (prim) {
                let ok = yield* doOneSlot(dlg, schema, slot, prim, scope);
                if (!ok)
                    return false;
            } else {
                if (slot.value.isEntity && slot.value.type === 'tt:username')
                    slot.value = yield* contactSearch(dlg, Type.Entity('tt:contact'), slot.value.value);
                if (!slot.value)
                    return false;
            }
        }
        for (let what of [program.query, program.action]) {
            if (!what.isSpecified)
                continue;
            for (let [schema, slot, prim, scope] of Generate.iterateSlotsFilter(what.schema, what.filter, what, {})) {
                let ok = yield* doOneSlot(dlg, schema, slot, prim, scope);
                if (!ok)
                    return false;
            }
