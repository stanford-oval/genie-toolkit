...
const Ast = require('thingtalk').Ast;
const { tryGetCurrentLocation, distance } = require('../utils');
...
async function lookupEntity(dlg, entityType, entityDisplay) {
    ...
    if (candidates.length === 0) {
        await dlg.reply(dlg._("Sorry, I cannot find any %s matching “%s”.").format(meta.name, entityDisplay));
        return null;
    }
...
async function lookupLocation(dlg, searchKey) {
    let candidates = await dlg.manager.thingpedia.lookupLocation(searchKey);
    ...
    if (candidates.length === 0) {
        await dlg.reply(dlg._("Sorry, I cannot find any location matching “%s”.").format(searchKey));
        return null;
    }
    ...
    if (currentLocation) {
        candidates.sort((l1, l2) => {
            return distance(l1, currentLocation.value) - distance(l2, currentLocation.value);
        });
    }
