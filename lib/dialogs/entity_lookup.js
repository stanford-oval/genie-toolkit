...
async function lookupEntity(dlg, entityType, entityDisplay) {
    ...
    if (candidates.length === 0) {
        dlg.reply(dlg._("Sorry, I cannot find any %s matching “%s”.").format(meta.name, entityDisplay));
        return null;
    }
...
async function lookupLocation(dlg, searchKey) {
    ...
    if (candidates.length === 0) {
        dlg.reply(dlg._("Sorry, I cannot find any location matching “%s”.").format(searchKey));
        return null;
    }
