...
module.exports = async function discoveryDialog(dlg, discoveryType, discoveryKind, discoveryName) {
    ...
    try {
        if (discoveryName !== undefined)
            await dlg.reply(dlg._("Searching for %s…").format(discoveryName));
        else
        ...
        if (e.code === 'ECANCELLED')
            throw e;
        await dlg.reply(dlg._("Discovery failed: %s").format(e.message));
        return;
    }
    ...
    if (devices.length === 0) {
        if (discoveryName !== undefined)
            await dlg.reply(dlg._("Can't find any %s around.").format(discoveryName));
        else
