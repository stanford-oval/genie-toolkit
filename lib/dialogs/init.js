...
function configureMessaging(dlg) {
    return dlg.manager.devices.factory.getFactory('org.thingpedia.builtin.omlet').then((factory) => {
        const delegate = {
            reply(msg) {
...
module.exports = function* initDialog(dlg, showWelcome) {
    ...
    if (initialized) {
        if (!dlg.manager.messaging.isAvailable) {
            ...
            dlg.reply(dlg._("You haven't configured an Omlet account yet. You need an Omlet account to let me talk to other Almonds."));
        ...
        dlg.reply(dlg._("First of all, I'll need you to configure an Omlet account."));
