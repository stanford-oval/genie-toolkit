...
module.exports = class Engine {
    constructor(platform) {
        ...
        if (platform.hasCapability('thingpedia-client'))
            thingpedia = platform.getCapability('thingpedia-client');
        else
            thingpedia = new ThingPediaHttpClient(platform.getDeveloperKey());

        if (hasApps) {
            this._schemas = new ThingTalk.SchemaRetriever(thingpedia);
        } else {
            this._schemas = null;
        }
        var deviceFactory = new DeviceFactory(this, thingpedia);
        ...
        if (hasDiscovery) {
            ...
            this._modules.push(new Discovery.Client(platform, this._devices, thingpedia));
        }
