...
module.exports = class Engine extends Tp.BaseEngine {
    ...
    startOAuth(kind) {
        return this._devices.addOAuth(kind);
    }
    ...
    getAppInfos() {
        const apps = this._apps.getAllApps();
        return apps.map((a) => this._toAppInfo(a));
    }

    async deleteApp(uniqueId) {
        const app = this._apps.getApp(uniqueId);
        if (app === undefined)
            return false;

        await this._apps.removeApp(app);
        return true;
    }

    async setCloudId(cloudId, authToken) {
        if (this._devices.hasDevice('thingengine-own-cloud'))
            return false;
        ...
        await this._tiers.reopenOne('cloud');
        return true;
    }
