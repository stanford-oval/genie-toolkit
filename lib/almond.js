...
module.exports = class Almond extends events.EventEmitter {
    ...
    notify(appId, icon, messages) {
        return this._dispatcher.dispatchNotify(appId, icon, messages);
    }

    notifyError(appId, icon, error) {
        return this._dispatcher.dispatchNotifyError(appId, icon, error);
    }

    askForPermission(principal, identity, program) {
        return this._dispatcher.dispatchAskForPermission(principal, identity, program);
    }

    askQuestion(appId, icon, type, question) {
        return this._dispatcher.dispatchAskQuestion(appId, icon, type, question);
    }

    interactiveConfigure(kind) {
        return this._dispatcher.dispatchInteractiveConfigure(kind);
    }

    runProgram(program) {
        return this._dispatcher.dispatchRunProgram(program);
    }
