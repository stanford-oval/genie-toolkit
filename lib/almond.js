...
module.exports = class Almond extends events.EventEmitter {
    ...
    handleParsedCommand(root) {
        ...
        return Intent.parse(root.code, root.entities, this.schemas, null, this._lastCommand, this._lastCandidates)
    ...
    _continueHandleCommand(command, analyzed) {
        ...
        return Promise.all(analyzed.candidates.map((candidate, beamposition) => {
            return Intent.parse(candidate.code, analyzed.entities, this.schemas, analyzed, this._lastCommand, this._lastCandidates).catch((e) => {
