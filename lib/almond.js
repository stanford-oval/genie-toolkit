...
const InitializationDialog = require('./dialogs/init');
...
module.exports = class Almond extends events.EventEmitter {
    ...
    notify(data) {
        if (!this._dialog || !this._dialog.notify(...data))
            this._queue.push(['notify', data, null]);
    }

    notifyError(data) {
        if (!this._dialog || !this._dialog.notifyError(...data))
            this._queue.push(['notifyError', data, null]);
    }
    ...
    askForPermission(data) {
        ...
        if (!this._dialog)
            ...
            promise = this._dialog.askForPermission(...data);

        if (promise)
            return promise;

        var defer = Q.defer();
        this._queue.push(['askForPermission', data, defer]);
    ...
    askQuestion(data) {
        ...
        if (!this._dialog)
            ...
            promise = this._dialog.askQuestion(...data);

        if (promise)
            return promise;

        var defer = Q.defer();
        this._queue.push(['askQuestion', data, defer]);
        return defer.promise;
    }

    interactiveConfigure(data) {
        if (!this._dialog || !this._dialog.interactiveConfigure(...data))
            this._queue.push(['interactiveConfigure', data, null]);
    }

    queueParsedCommand(command) {
        this._nextCommand.then(() => {
            if (!this._dialog || !(this._dialog instanceof DefaultDialog))
                this._queue.push(['handleParsedCommand', [command], null]);
            else
    ...
    _flushNotify() {
        while (this._queue.length > 0) {
            var [method, data, defer] = this._queue[0];
            if (method === 'handleParsedCommand') {
                if (this._dialog instanceof DefaultDialog) {
                    this._queue.shift();
                    this.handleParsedCommand(...data);
                    return;
                } else {
                    return;
                }
                ...
                try {
                    var result = this._dialog[method](...data);
                    if (result) {
                        this._queue.shift();
                        if (defer)
                    ...
                    if (defer)
                        ...
                        console.error('Error in queued ' + method + ': ' + e.message);
                }
            }
        }
    }
    ...
    prepare(dlg) {
        dlg.manager = this;
        dlg.gettext = this._;
        dlg.ngettext = this._ngettext;
        dlg.pgettext = this._pgettext;
        dlg._ = dlg.gettext;
        dlg.C_ = dlg.pgettext;
        dlg.N_ = N_;
    }
    ...
    _initialize() {
        ...
        this.setDialog(new StateMachineDialog(InitializationDialog, this._options.showWelcome));
    }
    ...
    handleParsedCommand(analyzed) {
        ...
        return this._nextCommand = this._nextCommand.then(() => {
            return this._doHandleCommand(analyzed, null, []);
    ...
    handleThingTalk(thingtalk) {
        ...
        return this._nextCommand = Intent.parseProgram(thingtalk, this.schemas).then((intent) => {
            return this._dialog.handle(intent);
        }).then(function(handled) {
            if (!handled)
                this._dialog.fail();
        }.bind(this)).catch(function(e) {
    ...
    _continueHandleCommand(command, analyzed) {
        ...
        if (choice !== null) {
            ...
            return this._dialog.handle(Intent.Fallback(command, fallbacks));
        } else {
            this._lastUtterance = command;
            this._lastCandidates = candidates;
            ...
            return this._dialog.handle(Intent.Failed(command));
        }
    }
