...
class ThingTalkExecutor {
    ...
    _findFunction(stmt) {
        ...
        return functions.length > 0 ? functions[functions.length-1] : undefined;
    }
    ...
    async executeState(state) {
        ...
        for (let i = 0; i < state.history.length; i++) {
            if (state.history[i].results !== null) {
                const fn = this._findFunction(state.history[i].stmt);
                if (!fn)
                    continue;
                resultList.push([fn, state.history[i].results.raw]);
                continue;
            }
            if (!this._isExecutable(state.history[i].stmt, state.history[i].confirm))
                continue;

            state.history[i].results = await this._executeStatement(state.history[i].stmt, resultList);
        }
    }
    ...
    async _iterateResults(app, intoRaw, intoProcessed) {
        let count = 0;
        if (app === null)
            return false;
        ...
        for (;;) {
            ...
            if (next.isNotification) {
                intoRaw.push([next.outputType, next.outputValue]);
                const mapped = await this._mapResult(next.outputType, next.outputValue);
                intoProcessed.push(new Ast.DialogueHistoryResultItem(null, mapped));
                count ++;
    ...
    async _executeStatement(stmt, previousResults) {
        ...
        const more = await this._iterateResults(app, rawResults, results);
