...
function describeArg(dlg, arg) {
    ...
    if (arg.isVarRef) {
        if (arg.name.startsWith('$contact:'))
            return arg.name.substring('$contact:'.length);
        switch (arg.name) {
...
function tryResolveSlotFromScope(slot, inScope) {
    var scopeIsEmpty = true;
    ...
    if (slot.name in inScope && typeCompat(inScope[slot.name].type, slot.type))
        return inScope[slot.name];

    // string is too generic of a type to do anything else
    if (slot.type.isString)
        return null;

    // count the number of values with this exact type in scope
    // if there is exactly one, we pick id
    // note that we use true type equality, not typeUnify, so we don't
    // match String against PhoneNumber, etc.

    var candidate = null;
    for (var name in inScope) {
        if (!slot.type.equals(inScope[name].type))
            continue;

        if (candidate !== null) {
            // multiple candidates, break
            candidate = null;
            break;
        }

        candidate = inScope[name];
    }
    if (candidate !== null)
        return candidate;

    // here we could see if there is an easy way to convert between types
    // and apply it
    // but we don't do it for now
    // we should revisit when we do true program synthesis

    return null;
}

function assignSlots(slots, prefilled, values, comparisons, fillAll, mustFill, inScope, toFill) {
    var outScope = {};
    ...
    slots.forEach((slot, i) => {
        ...
        if (!found) {
            var fromScope = tryResolveSlotFromScope(slot, inScope);

            if (fromScope !== null) {
                values[i] = Ast.Value.VarRef(fromScope.name);
                values[i].display = fromScope.canonical;
            } else {
                values[i] = undefined;
                if (fillAll || mustFill.has(slot.name) || slot.required)
                    toFill.push(i);
                else
                    outScope[slot.name] = slot;
            }
        }
    });
