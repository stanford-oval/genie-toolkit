...
function assignSlots(slots, prefilled, values, comparisons, fillAll, mustFill, inScope, toFill) {
    ...
    slots.forEach((slot, i) => {
        ...
        if (!found) {
            if (slot.name in inScope && typeCompat(inScope[slot.name], slot.type)) {
                values[i] = Ast.Value.VarRef(slot.name);
                values[i].display = slot.canonical;
            } else {
                values[i] = undefined;
                if (fillAll || mustFill.has(slot.name) || slot.required)
                    toFill.push(i);
                else
                    outScope[slot.name] = slot.type;
            }
        }
    });
