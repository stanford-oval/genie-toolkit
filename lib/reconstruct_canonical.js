...
module.exports = function reconstructCanonical(dlg, json) {
    ...
    if (analyzed.isRule) {
        ...
        return Q.all([triggerMeta, queryMeta, actionMeta]).spread(function(trigger, query, action) {
            ...
            if (trigger !== null) {
                ...
                triggerDesc = Describe.describeTrigger(dlg, analyzed.trigger.kind,
                                                       analyzed.trigger.channel,
                                                       analyzed.trigger.owner,
            ...
            if (query !== null) {
                ...
                queryDesc = Describe.describeAction(dlg, analyzed.query.kind,
                                                    analyzed.query.channel,
                                                    analyzed.query.owner,
            ...
            if (action !== null) {
                ...
                actionDesc = Describe.describeAction(dlg, analyzed.action.kind,
                                                     analyzed.action.channel,
                                                     analyzed.action.owner,
    ...
    return getMeta(schemaRetriever, analyzed, schemaType).then(function(meta) {
        // make up slots
        var scope = {};
        ...
        var comparisons = [];
        var toFill = [];
        ThingTalk.Generate.assignSlots(slots, analyzed.args, values, comparisons,
                            analyzed.isAction, analyzed.slots, scope, toFill);

        if (analyzed.isTrigger) {
            return dlg._("notify if %s").format(Describe.describeTrigger(dlg, analyzed.kind,
                                                                         analyzed.channel,
                                                                         analyzed.owner,
                                                                         meta, values, comparisons));
        } else {
            return Describe.describeAction(dlg, analyzed.kind,
                                           analyzed.channel,
                                           analyzed.owner,
                                           meta, values, comparisons);
        }
