...
class Paraphrase {
    constructor(schemaRetriever, tokenizer, language, counter, row) {
        this._schemas = schemaRetriever;
        this._tokenizer = tokenizer;
        this._language = language;
        this._noIdea = NO_IDEA[this._language] || [];
    ...
    async clean() {
        ...
        const { tokens, entities } = await this._tokenizer.tokenize(this._language, this.paraphrase);

        this.preprocessed = tokens;
...
module.exports = class ParaphraseValidator extends Stream.Transform {
    ...
    async _validate(row) {
        const paraphrase = new Paraphrase(this._schemas, this._tokenizer, this._language,
            this._counter, row);
