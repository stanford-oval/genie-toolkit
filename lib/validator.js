...
class ParaphraseValidator {
    constructor(schemaRetriever, tokenizer, locale, counter, row) {
    ...
    checkValues() {
        ...
        try {
            ...
            //console.log(String(this.id), e.message, this.preprocessed.join(' '), this.synthetic);
...
class ParaphraseValidatorFilter extends Stream.Transform {
    ...
    async _validate(row) {
        ...
        try {
            ...
            const out = {};
            out.id = row.id;
            out.preprocessed = paraphrase.preprocessed.join(' ');
            out.target_code = paraphrase.target_preprocessed.join(' ');
            if (row.context)
                out.context = paraphrase.context_preprocessed.join(' ');
