...
module.exports = class iBase extends events.EventEmitter {
    ...
    _transfer_filter(filter) {
        switch (filter.op) {
            case '=':  filter.op = '$eq'; break;
            case '!=': filter.op = '$ne'; break;
            case '>':  filter.op = '$gt'; break;
            case '<':  filter.op = '$lt'; break;
            case '>=': filter.op = '$gte'; break;
            case '<=': filter.op = '$lte'; break;
            case '~=':
                filter.op = '$regex';
            ...
            case '=~':
            default:
                throw new Error("Unsupported operator for log query: " + filter.op);
        }
        return filter;
    }
    ...
    _map(cols, filters, reduction) {
        ...
        filters.forEach((filter) => {
            filter = this._transfer_filter(filter);
            selector[filter.name] = {$exists: true, [filter.op]: filter.value}
        });
