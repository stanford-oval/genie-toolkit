...
class RequestHandler {
    constructor(stores, from, target, requestId, patterns, feed) {
        this._from = from;
        this._target = target;
        ...
        subjects.forEach((s) => {
            patterns.push({
                subject: s,
                predicate: 'http://thingengine.stanford.edu/ontology/0.1/#hasPermission',
                object: from
            });
        });
...
class ResponderFeed {
    constructor(db, messaging, feed) {
        ...
        this._feedUri = 'omlet://feed/' + encodeURIComponent(feed.feedId);
        this._userUri = 'omlet://user/' + messaging.account;
    ...
    _onIncomingMessage(message) {
        ...
        if (parsed.target.startsWith(this._feedUri) || parsed.target.startsWith(this._userUri))
            new RequestHandler(this._db, parsed.from, parsed.target, parsed.id, parsed.patterns, this._feed);
    }
