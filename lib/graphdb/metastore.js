...
module.exports = class MetaStore {
    ...
    _getDeviceStore(devices, uri) {
        var stores = devices.map(function(d) { return d.queryInterface('rdf'); });
        if (stores.length === 0)
    ...
    _getMyStore(rest) {
        if (rest === null || rest[1].length === 1)
            return this._getMeStore();
        else
            return this._getDeviceStore(this._getDevices(rest[1].substr(1)), Constants.ME + rest);
    }
