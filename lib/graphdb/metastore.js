...
module.exports = class MetaStore {
    ...
    _getDeviceStores(devices) {
        return devices.map(function(d) { return d.queryInterface('rdf'); }).filter((s) => s !== null);
    }

    _getDeviceStore(devices, uri) {
        var stores = this._getDeviceStores(devices);
        if (stores.length === 0)
    ...
    _getMeStore() {
        ...
        var deviceStores = this._getDeviceStores(this._devices.values());
