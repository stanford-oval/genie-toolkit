...
module.exports = class ConfigDialog extends Dialog {
    ...
    _promptConfigure() {
        return this.manager.thingpedia.getDeviceSetup([this.kind]).then((factories) => {
            ...
            if (!factory) {
                this.reply("I'm so sorry, I can't find " + this.kind + " in my database.");
                return false;
            }
            
            if (factory.type === 'none') {
                this.reply(this.kind + " doesn't need configuration.");
                return false;
                ...
                if (factory.choices.length > 0) {
                    ...
                    this.reply("Sorry, I don't know how to configure " + this.kind + ".");
                }
            } else {
                this.reply("OK, here's the link to configure " + this.kind);
                switch (factory.type) {
                    case 'oauth2':
    ...
    handle(command) {
        ...
        if (this.kind === 'generic') {
            this.reply("You can configure devices by command 'configure $device', e.g., 'configure twitter'.");
            this.switchToDefault();
            return true;
        }
        
        return this._promptConfigure().then(() => {
            this.switchToDefault();
            return true;
        });
    }
