...
module.exports = class RuleDialog extends Dialog {
    ...
    execute() {
        Q.try(() => {
            return Codegen.codegenRule(this.manager.schemas, this.trigger, this.action);
        }).then((code) => {
            var name = this.capitalizeSelector(this.trigger.kind, this.trigger.channel) + ' to ' +
    ...
    _continue(command) {
        ...
        if (SlotFillingDialog.slotFill(this, this.action, true, this.trigger.slots))
            return true;
