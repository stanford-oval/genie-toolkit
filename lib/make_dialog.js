...
module.exports = class MakeDialog extends Dialog {
    ...
    handle(command) {
        return this.handleGeneric(command).then((handled) => {
            ...
            if (this.expecting === ValueCategory.MultipleChoice) {
                if (this._helpState === 'filterList') {
                    var idx = this.choices.indexOf(this._commandClass);
                    if (command.value < 0 || command.value > this.filterCandidates[idx].length)
                        return this.unexpected();
                    if (command.value === this.filterCandidates[idx].length)
                        return this._handleBack();
                    var filter = this.filterCandidates[idx][command.value];
                    this.filters[idx].push(filter);
                    this.filterDescription[idx] += ", " + [filter.name, filter.operator, filter.value].join(' ');
                    return this._handleMakeRule();
                } else if (this._helpState === 'filterCommandList') {
    ...
    _getFilterList() {
        ...
        this.ask(ValueCategory.MultipleChoice, "Pick the filter you want to add:");
        var idx = this.choices.indexOf(this._commandClass);
        this.filterCandidates[idx].forEach((filter, i) => {
            this.replyChoice(i, "choice", [filter.name, filter.operator, "____"].join(' '));
        });
        this.replyChoice(this.filterCandidates[idx].length, "choice", "Back");
        return true;
    }
