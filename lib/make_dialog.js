...
module.exports = class MakeDialog extends Dialog {
    ...
    handle(command) {
        return this.handleGeneric(command).then((handled) => {
            ...
            if (command.isMake || (command.isHelp && command.name === null)) {
            ...
            if (this.expecting === ValueCategory.Command) {
                ...
                if (command.isPrimitive || command.isEmpty) {
                ...
                if (command.isFilter) {
                    ...
                    if (filter.value === null) {
                        ...
                        switch (filter.type) {
                            ...
                            case 'Bool': return this.ask(ValueCategory.YesNo, question);
            ...
            if (this.expecting === ValueCategory.Number || this.expecting === ValueCategory.EmailAddress ||
                this.expecting === ValueCategory.PhoneNumber || this.expecting === ValueCategory.YesNo) {
            ...
            if (this.expecting && this.expecting.isMeasure) {
                var filter = this._currentFilter;
                this._currentFilter = null;
                filter.value = command.value.value;
    ...
    _handleMakeRule() {
        this._helpState = null;
        var hint = this._("Click on one of the following buttons to start adding command.");
        var hintWhenReady = this._("Add more commands and filters or run your command if you are ready.");
        this.ask(ValueCategory.MultipleChoice, this.ready? hintWhenReady : hint);
    ...
    _getFilterList() {
        ...
        this.filterCandidates[idx].forEach((filter, i) => {
            this.replyButton(
                [filter.name.replace('_', ' '), filter.operator, '____'].join(' '),
    ...
    _replyBack() {
        return this.replyButton(this._("Back"), JSON.stringify({ special: 'tt:root.special.back' }));
    }

    _replySpecial() {
        var special =  JSON.stringify({ special: 'tt:root.special.empty' });
    ...
    _addFiltersFromJson() {
        ...
        if (json[channel].person)
            this.filterDescription[idx] += ", owner is " + json[channel].person;
            ...
            for (var j = 0; j < this.first.schema.args.length; j ++ ) {
                if (String(this.first.schema.schema[j]) === String(this.second.schema.schema[i])) {
                    options.push({
                        ...
                        text: this._("Use the %s from %s").format(argToEnglish(this.first.schema.args[j]), this.first.kind.replace(/_/g, ' '))
                    });
                }
            }
                ...
                for (var j = 0; j < this.second.schema.args.length; j ++ ) {
                    if (String(this.second.schema.schema[j]) === String(this.third.schema.schema[i])) {
                        options.push({
                            ...
                            text: this._("Use the %s from %s").format(argToEnglish(this.second.schema.args[j]), this.second.kind.replace(/_/g, ' '))
                        });
                    }
                }
