...
module.exports = class Sabrina extends events.EventEmitter {
    ...
    expect(expecting) {
        ...
        this._raw = (expecting === ValueCategory.RawString || expecting === ValueCategory.MultipleChoice);
    }
    ...
    _initialize() {
        if (this._initialized) {
            this.sendReply(this._("Welcome back, %s!").format(this.user.name));
            // ignore the dialog status in last session (should we?)
    ...
    _continueHandleCommand(command, analyzed) {
            ...
            if (top === '{"special":{"id":"tt:root.special.failed"}}') {
                ...
                for (var candidate of analyzed) {
                    if (fallbacks.length >= 5)
                        break;
                    if (candidate.prob < 0.15 || candidate.score < -20)
                        break;
        ...
        if (choice !== null) {
            ...
            var analyzer = new SemanticAnalyzer(parsed, command);
    ...
    handleCommand(command) {
        ...
        return Q.try(() => {
            ...
            return this.sempre.sendUtterance(command, this._expecting).then((analyzed) => {
