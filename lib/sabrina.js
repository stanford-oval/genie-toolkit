...
module.exports = class Sabrina extends events.EventEmitter {
    constructor(engine, user, delegate, debug, sempreUrl) {
        ...
        this._sempre = new SempreClient(sempreUrl, engine.platform.locale);
    ...
    handleParsedCommand(analyzed) {
        ...
        return Q.try(() => {
            return this._continueHandleCommand(analyzed);
    ...
    _continueHandleCommand(analyzed) {
        if (analyzed === null) {
    ...
    handleCommand(command) {
        ...
        return Q.try(() => {
            ...
            return this._sempre.sendUtterance(command).then((analyzed) => {
                return this._continueHandleCommand(analyzed);
            });
        }).then((handled) => {
            if (!handled)
