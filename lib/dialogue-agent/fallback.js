...
async function getFallbackExamples(dlg, command) {
    const dataset = await dlg.conversation.thingpedia.getExamplesByKey(command);
    const examples = ENABLE_SUGGESTIONS ? await Helpers.loadExamples(dataset, dlg.conversation.schemas, 5) : [];
    ...
    dlg.conversation.stats.hit('sabrina-fallback-buttons');
