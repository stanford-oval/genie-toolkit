...
const RuleExecutor = require('../../engine/apps/rule_executor');
...
async function lookupEntity(dlg, entityType, entityDisplay, idEntities) {
    ...
    if (candidates.length === 0) {
        ...
        try {
            ...
            var invocation = (new Ast.Invocation(null, new Ast.Selector.Device(null, kind, null, null), query, [], schema));
            ...
            const program = new Ast.Program(null, [], [],
                [new Ast.Statement.Command(null, filteredTable, [Ast.Action.notifyAction()])], null);
            const compiler = new AppCompiler(dlg.manager.schemas);
            const compiled = await compiler.compileProgram(program);
            const app = await dlg.engine.createApp(program);
            const mainOutput = new QueueOutputDelegate();
            const command = new RuleExecutor(dlg.engine, app, compiled.command, mainOutput);
            command.start();
            await command.waitFinished();
            var result = await mainOutput.next();
            const candidate = [{
                type: entityType,
                value: result.value.outputValue.id.value,
                canonical: result.value.outputValue.id.display,
                name: result.value.outputValue.id.display
            }];
            return getBestEntityMatch(entityDisplay, entityType, candidate);
        } catch (error) {
