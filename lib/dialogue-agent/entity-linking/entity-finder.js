...
function getBestEntityMatch(searchTerm, entityType, candidates) {
    ...
    searchTerm = _removeParenthesis(searchTerm);
    let searchTermTokens = searchTerm.split(' ');

    for (let cand of candidates) {
        let candDisplay = _removeParenthesis(cand.canonical);
        ...
        for (let candToken of candTokens) {
            ...
            for (let token of searchTermTokens) {
                if (editDistance(token, candToken) <= 1 && token.length > 1 || editDistance(token, candToken) === 0) {
                    score += 10;
                    found = true;
...
function _removeParenthesis(str){
