...
function addActionParameter(nextInvocation, dialogueAct, pname, value, confirm) {
...
function computeStateInfo(state) {
    let nextItem = null, nextInfo = null, previousItem = null, currentItem = null;
    ...
    return [nextItem, nextInfo, previousItem, currentItem];
}
...
function getProjection(historyItem) {
    if (!historyItem.stmt.table)
        return null;
    let projection = historyItem.stmt.table.isProjection ? historyItem.stmt.table.args.slice() : null;
    if  (projection)
        projection.sort();
    return projection;
}
...
function isUserAskingResultQuestion(previousItem, currentItem) {
    ...
    // and the projection was different at the previous turn (or there was no projection at the previous turn)

    if (!currentItem || !previousItem)
        return false;

    let currentProjection = getProjection(currentItem);
    if (!currentProjection)
        return false;

    let previousProjection = getProjection(previousItem);
    ...
    return !arraySubset(currentProjection, previousProjection);
}
...
module.exports = {
    async chooseAction(dlg, state) {
        ...
        const [nextItem, nextInfo, previousItem, currentItem] = computeStateInfo(state);
        ...
        case 'cancel':
            return makeSimpleDialogueAct('sys_anything_else');

        case 'end':
            return makeSimpleDialogueAct('sys_end');
        ...
        case 'execute': {
            ...
            const results = currentItem.results.results;
            ...
            if (results.length === 1) {
                // we must recommend
                if (nextItem && !isUserAskingResultQuestion(previousItem, currentItem)) {
                    const nextInvocation = getActionInvocation(nextItem);
                    return addActionParameter(nextInvocation, 'sys_recommend_one', nextInfo.chainParameter, results[0].value.id, 'proposed');
                } else {
    ...
    async reply(dlg, state) {
        ...
        case 'sys_empty_search_question':
            if (state.dialogueActParam) {
                const schema = currentItem.stmt.table.schema;
                ...
                await dlg.replyInterp(dlg._("There are no ${table}. How about a different ${argcanonical}?"), {
                    ...
                    table: describer.describeTable(currentItem.stmt.table, [])
                });
                return;
            }
        ...
        case 'sys_recommend_one':
            if (isUserAskingResultQuestion(previousItem, currentItem)) {
                // answer the question directly
                for (let result of currentItem.results.results)
        ...
        case 'sys_learn_more_what':
            await dlg.reply(dlg._("What would you like to know?"));
            break;

        case 'sys_anything_else':
            await dlg.reply(dlg._("Is there anything else I can help you with?"));
            break;

        case 'sys_end':
            await dlg.reply(dlg._("Alright, good bye!"));
            break;
    ...
    getInteractionState(dlg) {
        return {
            isTerminal: dlg.dialogueAct === 'sys_end',
