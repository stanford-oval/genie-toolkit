...
module.exports = class GenieBasedPolicy {
    constructor(dlg, templateFiles) {
        this._dlg = dlg;

        this._langPack = Genie.I18n.get(dlg.locale);

        assert(dlg.rng);
        this._sentenceGenerator = new Genie.SentenceGenerator({
            contextual: true,
            rootSymbol: '$agent',
            flags: {
                // FIXME
