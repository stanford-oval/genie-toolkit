...
export function formatError(dlg, error) {
...
export function presentExample(dlg, utterance) {
...
export function loadOneExample(ex) {
...
export async function loadExamples(dataset, schemaRetriever, maxCount) {
...
export function presentExampleList(dlg, examples) {
    for (let ex of examples)
        dlg.replyButton(presentExample(dlg, ex.utterance), ex.target);
}

export function presentSingleExample(dlg, utterance, target) {
    ...
    if (target.slots && target.slots.length > 0) {
        dlg.replyButton(presentExample(dlg, utterance), target);
    } else {
