...
async function failWithFallbacks(dlg, command, fallbacks) {
    ...
    if (countCanonicals === 0) {
        ...
        let ok = await dlg.ask(ValueCategory.YesNo, dlg._("Did you mean ${command}?"), { command: target_canonical });
        if (ok) {
            return target;
        } else {
            dlg.reset();
...
async function fallback(dlg, intent) {
    ...
    if (learn) {
        ...
        await dlg.replyInterp(dlg._("${count:plural:\
            one {You have trained me with ${count} sentence.}\
            other {You have trained me with ${count} sentences.}\
        }"), { count });
    }
