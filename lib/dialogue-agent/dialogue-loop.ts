...
export class DialogueLoop {
    ...
    constructor(conversation : Conversation,
        ...
        this._handlers = {
            thingtalk: new ThingTalkDialogueHandler(engine, this, this._agent, this._nlu, this._nlg, options)
        };
    ...
    getState() : Record<string, unknown> {
        ...
        for (const key in this._handlers)
            state[key] = this._handlers[key].getState();
        return state;
    }
    ...
    private async _handleUICommand(type : CommandAnalysisType) {
        ...
        case CommandAnalysisType.DEBUG:
            ...
            for (const key in this._handlers)
                await this.reply(key + ': ' + this._handlers[key].getState());
            break;
        }
    }
    ...
    private async _handleAPICall(call : QueueItem) {
        if (call instanceof QueueItem.Notification)
            await this._sendAgentReply(await this._handlers.thingtalk.showNotification(call.app, call.outputType, call.outputValue));
        else if (call instanceof QueueItem.Error)
            await this._sendAgentReply(await this._handlers.thingtalk.showAsyncError(call.app, call.error));
    }
    ...
    private async _initialize(showWelcome : boolean, initialState : Record<string, unknown>|null) {
        ...
        for (const key in this._handlers) {
            const handler = this._handlers[key];
            const reply = await handler.initialize(initialState ? initialState[key] : undefined, showWelcome);
    ...
    setExpected(expected : ValueCategory|null, raw = (expected === ValueCategory.RawString || expected === ValueCategory.Password)) {
        ...
        this.raw = raw;
        const [contextCode, contextEntities] = this._handlers.thingtalk.prepareContextForPrediction();
    ...
    async ask(expected : ValueCategory.YesNo|ValueCategory.PhoneNumber|ValueCategory.EmailAddress|ValueCategory.Location|ValueCategory.Time,
        ...
        let analyzed = await this._handlers.thingtalk.analyzeCommand(await this.nextCommand());
        while (analyzed.answer === null || typeof analyzed.answer === 'number' ||
            ...
            analyzed = await this._handlers.thingtalk.analyzeCommand(await this.nextCommand());
        }
