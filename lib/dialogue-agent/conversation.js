...
module.exports = class Conversation extends events.EventEmitter {
    ...
    async start() {
        ...
        await this._loop.init();
        this._resetInactivityTimeout();
        return this._loop.start(this._options.showWelcome, this._options.configureMessaging);
    }
    ...
    _sendUtterance(utterance) {
        ...
        return this._nlu.sendUtterance(utterance, contextCode, contextEntities, {
    ...
    _resetInactivityTimeout() {
        ...
        this._inactivityTimeout = setTimeout(() => {
            // reset the context on inactivity
            this.setContext(null);

            // inform the app that the conversation is inactive (will close the connection, turn off LEDs, etc.)
            this.emit('inactive');
    ...
    async sendAskSpecial() {
        ...
        return this._addMessage({ type: MessageType.ASK_SPECIAL, askSpecialType: what });
    }
};
