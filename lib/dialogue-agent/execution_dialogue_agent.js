...
const StatementExecutor = require('./statement_executor');
const { CancellationError } = require('./errors');
const Helpers = require('./helpers');

const AbstractDialogueAgent = require('./abstract_dialogue_agent');
 ...
 */
class ExecutionDialogueAgent extends AbstractDialogueAgent {
    ...
    async lookupEntityCandidates(entityType, entityDisplay) {
        ...
        const { data: candidates, meta } = await this._thingpedia.lookupEntity(entityType, entityDisplay);
        ...
        try {
            ...
            const stmt = await this._constructEntityQuery(kind, query, entityDisplay);
            await this._prepareForExecution(stmt);
...
module.exports = ExecutionDialogueAgent;
