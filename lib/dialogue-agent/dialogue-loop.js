...
const { getPrimitiveIcon, getProgramIcon } = require('../utils/icons');
...
module.exports = class DialogueLoop {
    constructor(manager, engine, debug) {
        ...
        this._policy = new DialoguePolicy(this);
    ...
    async _doAgentReply() {
        let policyPrediction, expect, utterance;
        if (this._useNeuralNLG()) {
            [policyPrediction, expect] = await this._policy.chooseAction(this._dialogueState);
            ...
            await this.manager.setContext(context, { allocateEntities: true, typeAnnotations: false });

            utterance = await this.manager.generateAnswer(policyPrediction);
        } else {
            [policyPrediction, expect, utterance] = await this._policy.chooseAction(this._dialogueState);
        }
    ...
    async prepareForExecution() {
        ...
        const dialogueAct = this._dialogueState.dialogueAct;
        const dialogueActParam = this._dialogueState.dialogueActParam;
        const hints = collectDisambiguationHintsForState(this._dialogueState);

        for (let slot of this._dialogueState.iterateSlots2()) {
            if (slot instanceof Ast.Selector) {
                this._dialogueState.dialogueAct = 'sys_ask_device';
                this._dialogueState.dialogueActParam = null;
                let ok = await chooseDevice(this, slot);
                if (!ok)
                    return false;
            } else {
                this._dialogueState.dialogueAct = 'sys_ask_concretize_value';
                this._dialogueState.dialogueActParam = null;
                this.icon = getPrimitiveIcon(slot.primitive);
        ...
        this._dialogueState.dialogueAct = dialogueAct;
        this._dialogueState.dialogueActParam = dialogueActParam;

        return true;
    }

    async execute() {
        [this._dialogueState, this._executorState] = await this._executor.execute(this._dialogueState, this._executorState);
    }
    ...
    setExpected(expected) {
        ...
        this.manager.setContext(context, { typeAnnotations: false });
