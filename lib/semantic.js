...
module.exports = class SemanticAnalyzer {
    constructor(obj) {
        ...
        } else if ('$$failed' in this.root) {
            this.isSpecial = true;
            this.isFailed = true;
            this.special = 'tt:root.special.failed';
            this.raw = this.root.$$failed;
        } else if ('$$fallback' in this.root) {
            this.isSpecial = true;
            this.isFallback = true;
            this.special = 'tt:root.special.fallback';
            this.raw = this.root.$$fallback.raw;
            this.fallbacks = this.root.$$fallback.fallbacks;
        } else {
    ...
    static makeFailed(raw) {
        return new SemanticAnalyzer({ $$failed: raw });
    }

    static makeFallbacks(raw, fallbacks) {
        return new SemanticAnalyzer({ $$fallback: { raw: raw, fallbacks: fallbacks }});
    }
