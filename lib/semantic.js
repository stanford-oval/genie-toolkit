...
Intent.parse = function parse(root, schemaRetriever, raw, previousRaw, previousCandidates) {
    ...
    if ('special' in root) {
        ...
        intent = SEMPRESyntax.parseRule(schemaRetriever, root.rule, true).then((prog) => new Intent.Program(raw, prog));
        ...
        intent = SEMPRESyntax.parseRule(schemaRetriever, root.setup, true)
            .then((prog) => new Intent.Setup(raw, Ast.Value.Entity(root.setup.person, 'tt:contact_name', null), prog));
    }
