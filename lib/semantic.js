...
function parseLocation(loc) {
    if (loc.relativeTag === 'absolute')
        return Ast.Value.Location(Ast.Location.Absolute(loc.longitude, loc.latitude, null));
    else
...
function parseValue(value) {
    ...
    if (type.isEntity) {
        parsed = Ast.Value.Entity(value.value.value, type.type, value.value.display||null);
    } else if (type.isString) {
