...
module.exports = class SemanticAnalyzer {
    constructor(json, raw, previousRaw, previousCandidates) {
        ...
        if ('special' in this.root) {
            ...
            } else if (special === 'tt:root.special.makerule') {
                this.isMake = true;
            } else if (special === 'tt:root.special.empty') {
                this.isEmpty = true;
            } else if (special === 'tt:root.special.back') {
                this.isBack = true;
            } else {
                this.isSpecial = true;
                this.special = special;
            ...
            this.trigger = null;
            this.query = null;
            this.action = this._handlePrimitive(this.root.action);
            // for compatibility with some ugly code in makedialog
            this.action.root = this.root;
        } else if ('trigger' in this.root) {
            this.isRule = true;
            this.isPrimitive = true;
            this.once = false; // FIXME
            this.trigger = this._handlePrimitive(this.root.trigger);
            this.query = null;
            this.action = null;
            // for compatibility with some ugly code in makedialog
            this.trigger.root = this.root;
        } else if ('query' in this.root) {
            this.isRule = true;
            this.isPrimitive = true;
            this.once = false;
            this.trigger = null;
            this.query = this._handlePrimitive(this.root.query);
            this.action = null;
            // for compatibility with some ugly code in makedialog
            this.query.root = this.root;
        } else if ('rule' in this.root) {
            this.isRule = true;
            this.once = !!this.root.rule.once;
            if (this.root.rule.trigger)
                this.trigger = this._handlePrimitive(this.root.rule.trigger);
            else
                this.trigger = null;
            if (this.root.rule.query)
                this.query = this._handlePrimitive(this.root.rule.query);
            else
                this.query = null;
            if (this.root.rule.action)
                this.action = this._handlePrimitive(this.root.rule.action);
            else
                this.action = null;
        ...
        } else if ('setup' in this.root) {
            this.isSetup = true;
            this.person = Ast.Value.VarRef('$contact(' + this.root.setup.person + ')');
            this.rule = this.root.setup;
            delete this.rule.person;
        } else {
