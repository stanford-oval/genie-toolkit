...
Intent.parse = function parse(root, schemaRetriever, raw, previousRaw, previousCandidates) {
    ...
    if ('special' in root) {
        ...
        return SEMPRESyntax.parseRule(schemaRetriever, root.setup, true).then((prog) => {
            prog.principal = Ast.Value.Entity(root.setup.person, 'tt:contact_name', null);
            return new Intent.Setup(raw, prog);
        });
