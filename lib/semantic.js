...
module.exports = class SemanticAnalyzer {
    constructor(obj) {
        ...
        if ('special' in this.root) {
        ...
        } else if ('value' in this.root) {
            this.isValue = true;
            this._handleValue(this.root.value);
            ...
            this.args = this.root.action.args;
            ...
            this.trigger = {
                ...
                kind: trigger[0],
                channel: trigger[1],
                args: this.root.rule.trigger.args
            };
            ...
            this.action = {
                ...
                kind: action[0],
                channel: action[1],
                args: this.root.rule.action.args
            };
        } else {
    ...
    _handleSelector(sel) {
        var match = '/^tt:([a-z0-9A-Z]+)\.([a-z0-9A-Z]+)$/'.match(sel);
        if (match === null)
