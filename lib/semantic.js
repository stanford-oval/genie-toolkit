...
function parseLocation(loc) {
    if (loc.relativeTag === 'absolute')
        ...
        return Ast.Value.VarRef('$location.' + loc.relativeTag.substr('rel_'.length));
}
...
function valueToCategoryAndValue(value) {
    ...
    case 'Picture':
        return [ValueCategory.Picture,
                Ast.Value.Picture(value.value.value),
                'picture'];
    case 'Time':
        var time = parseTime(value.value);
        return [ValueCategory.RawString,
                Ast.Value.String(time), time];
    case 'Date':
    ...
    case 'PhoneNumber':
        return [ValueCategory.PhoneNumber,
                Ast.Value.PhoneNumber(value.value.value),
                value.value.display || value.value.value];
    case 'EmailAddress':
        return [ValueCategory.EmailAddress,
                Ast.Value.EmailAddress(value.value.value),
                value.value.display || value.value.value];
    case 'Contact':
...
module.exports = class SemanticAnalyzer {
    constructor(obj, raw, previousRaw, previousCandidates) {
        this.root = obj;
        this.raw = raw;
        ...
        if ('special' in this.root) {
            ...
            // \r {"command": {"type":"discover", "value":{"id":"tt:device.hue"}}}
            // \r {"command": {"type":"list", "value":{"value":"device"}}}
            // \r {"command": {"type":"help", "value":{"id":"tt:device.twitter"}}}
            // \r {"command": {"type":"setting", "value":{"name":"alice tester"}}}
            ...
            switch (this.root.command.type) {
                case 'action':
                    ...
                    return;
                case 'discover':
                    var name = this._handleName(this.root.command.value);
                ...
                case 'setting':
                    this.isSetting = true;
                    this.name = this._handleName(this.root.command.value.name);
                    return;
    ...
    static makeFallbacks(raw, fallbacks) {
        return new SemanticAnalyzer({ $$fallback: fallbacks }, raw);
    }
