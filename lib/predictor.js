...
class Worker extends events.EventEmitter {
    ...
    start() {
        ...
        if (process.env.DECANLP_THINGPEDIA)
            args.push('--thingpedia', process.env.DECANLP_THINGPEDIA);
        else
            args.push('--thingpedia', path.resolve(this._modeldir, 'thingpedia.json'));
    ...
    request(tokens) {
        ...
        if (process.env.DECANLP_TASK_NAME === 'almond_with_thingpedia_as_context') {
            this._stream.write({
                id,
                context: '',
                question: tokens.join(' '),
                task: process.env.DECANLP_TASK_NAME
            });
        } else {
            this._stream.write({
                id,
                ...
                task: process.env.DECANLP_TASK_NAME || 'almond'
            });
        }
