...
module.exports = class ContactSearchDialog extends Dialog {
    ...
    static resolve(parent, type, values, index) {
        ...
        if (parent.subdialog === null) {
            parent.push(new ContactSearchDialog(values[index].name.substr('$contact:'.length), type));
    ...
    handleRaw(raw) {
        if (this.choices !== null &&
            ...
            for (var e of this.choices) {
                if (e.displayName === raw.toLowerCase().trim()) {
                    this.expecting = null;
                    this._learn(e);
                    this.resolved = this._makeValue(e);
                    return false;
                }
            }
