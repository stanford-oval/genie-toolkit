...
module.exports = class ContactSearchDialog extends Dialog {
    ...
    _makeValue(choice) {
        var value;
        if (this.category === ValueCategory.PhoneNumber)
            value = Ast.Value.Entity(choice.value, 'tt:phone_number');
        else if (this.category === ValueCategory.EmailAddress)
            value = Ast.Value.Entity(choice.value, 'tt:email_address');
        else
            value = Ast.Value.Entity(choice.value, 'tt:contact');
    ...
    continue() {
        ...
        return this._lookupContact().then((result) => {
            ...
            if (result.length === 1) {
                this.resolved = this._makeValue(result[0]);
                return false;
            }
