...
export default class LanguagePack {
    ...
    postprocessNLG(answer : string, entities : EntityMap, delegate : UnitPreferenceDelegate) : string {
        ...
        // GIANT ENORMOUS HACK: send the answer through to gettext to do a final attempt
        // at translation
        const zip = /Alright, I will notify you when there are new appointments in the ([0-9]+) zip code./.exec(answer);
        if (zip) {
            answer = this._(answer.replace(/[0-9]+/, '${zip_code}'))
                .replace('${zip_code}', zip[1]);
        } else {
            answer = this._(answer);
        }
