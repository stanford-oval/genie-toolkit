...
module.exports = class ChineseTokenizer extends BaseTokenizer {
    _addIntlPrefix(text) {
        // assume PRC
        if (!text.startsWith('+'))
            text = '+86' + text;
    ...
    _parseWordNumber(text) {
        ...
        // the logic is similar to other languages though: you operate a buffer and multiply and add
    ...
    _initNumbers() {
        ...
        // currencies
        this._lexer.addRule(/{DECIMAL_NUMBER}(?:[美加澳欧]?元|英镑|日圆)/, (lexer) => {
            let unitlength = /[美加澳欧]元|英镑|日圆/.test(lexer.text) ? 2 : 1;
    ...
    _parseWordDate(text, parseTime) {
        const day = this._findAndParseNumberOffset(/(三十一?|[一二]十[一二三四五六七八九]?|[一二三四五六七八九])号/, /(3[01]|[12][0-9]|[1-9])号/, text, -1);
        ...
        if (parseTime) {
            ...
            let dateend = text.indexOf('号');
            if (dateend < 0)
                dateend = text.indexOf('月');
            const time = parseTime(text.substring(dateend+1));
    ...
    _initDates() {
        ...
        this._addDefinition('DAY_NAME', /星期[一二三四五六日天]/);

        this._addDefinition('DAY', /(?:三十一?|[一二]十[一二三四五六七八九]?|[一二三四五六七八九]|3[01]|[12][0-9]|[1-9])号/);
        this._addDefinition('MONTH', /(?:一?十[一二]|[一二三四五六七八九]|1[0-2]|[1-9])月/);
        this._addDefinition('YEAR', /[〇一二三四五六七八九0-9]{4}年/);

        // dates with words

        this._lexer.addRule(/(?:{YEAR}?{MONTH}{DAY}|{YEAR}{MONTH})/, (lexer) => {
        ...
        // date and time
        this._lexer.addRule(/(?:{YEAR}?{MONTH}{DAY}|{YEAR}{MONTH})在?{PLAIN_TIME}/, (lexer) => {
        ...
        this._lexer.addRule(/(?:{YEAR}?{MONTH}{DAY}|{YEAR}{MONTH})在?{COLLOQUIAL_TIME}/, (lexer) => {
