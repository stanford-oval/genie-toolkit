...
class Grammar extends events.EventEmitter {
    ...
    generate(contexts, callback) {
        ...
        for (let depth = 0; depth <= this._options.maxDepth; depth++) {
            ...
            charts[depth] = [];
            for (let index = 0; index < this._nonTermList.length; index++)
                charts[depth].push(new ReservoirSampler(targetPruningSize, this._options.rng));
            ...
            for (let index = 0; index < this._nonTermList.length; index++) {
                ...
                for (let rulenumber = 0; rulenumber < this._rules[index].length; rulenumber++) {
                    ...
                    expandRule(charts, depth, index, rulenumber, rule, this._averagePruningFactor, this._options, this._nonTermList, (derivation) => {
                        if (derivation === null)
                            return;
...
function expandRule(charts, depth, nonTermIndex, rulenumber, [expansion, combiner], averagePruningFactor, options, nonTermList, emit) {
    ...
    const coinProbability = Math.min(1, 1000000/worstCaseGenSize);
