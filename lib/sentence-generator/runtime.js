...
const DEPTH_PROGRESS_MULTIPLIERS = [
    50, 1500, 21000, 1350000, 750000, 400000, 3000000, 3000000, 3000000
];
...
class Grammar extends events.EventEmitter {
    ...
    addConstants(symbol, token, type, prefix = '', suffix = '') {
        if (this._finalized)
        ...
        for (let i = 0; i < (this._options.maxConstants || DEFAULT_MAX_CONSTANTS); i++) {
            ...
            expansion.push(constant);
            if (suffix)
                expansion.push(suffix);

            this._rules[symbol].push([expansion, () => new Derivation(constant.value, expansion)]);
        }
    }
