...
class Rule {
    constructor(number, expansion, combiner, weight = 1) {
        this.number = number;
...
class Grammar extends events.EventEmitter {
    ...
    _addRuleInternal(symbolId, expansion, combiner, attributes = {}) {
        ...
        this._rules[symbolId].push(new Rule(rulenumber, expansion, combiner, attributes.weight));
    }
        ...
        for (let depth = 0; depth <= this._options.maxDepth; depth++) {
            ...
            for (let index = 0; index < this._nonTermList.length; index++) {
                ...
                for (const rule of this._rules[index]) {
                    ...
                    let sampler = new ReservoirSampler(Math.floor(targetPruningSize * rule.weight), this._options.rng);
