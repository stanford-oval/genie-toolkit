...
class Derivation {
    ...
    replacePlaceholder(name, derivation, semanticAction, { isConstant, isUndefined = false, throwIfMissing = false }) {
        let newValue;
        let isDerivation;
        if (!(derivation instanceof Derivation)) {
            ...
            assert(contextCompatible(this.context, derivation.context));
    ...
    _typecheck() {
        ...
        for (let nonTermIndex = 0; nonTermIndex < this._nonTermList.length; nonTermIndex++) {
            ...
            for (let rule of rules) {
                ...
                if (rule.hasContext && rule.expansion.length === 1)
                ...
                if (expansion[k] instanceof NonTerminal) {
                    for (let candidate of charts[fixeddepth][expansion[k].index]) {
                        ...
                        if (candidate instanceof Context) {
                            ...
                            if (!contextCompatible(context, candidate.context, context))
                                continue;
