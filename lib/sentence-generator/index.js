    ...
    _recordOutputParam(pname, ptype, arg) {
        ...
        if (expansion) {
            for (let candidate of expansion) {
                this._grammar.declareSymbol('out_param_npp');
                this._grammar.addRule('out_param_' + typestr, [candidate], $runtime.simpleCombine(() => new Ast.Value.VarRef(pname)));
                this._grammar.addRule('out_param_npp', [candidate], $runtime.simpleCombine(() => new Ast.Value.VarRef(pname)));
            }
        } else {
        ...
        for (let preprocessed of ex.preprocessed) {
            ...
            if (grammarCat === 'thingpedia_query') {
                if (ex.value.schema.hasArgument('id')) {
                    ...
                    if (type.isEntity && type.type === 'org.wikidata:human')
                        grammarCat = 'thingpedia_who_question';
