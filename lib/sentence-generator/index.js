...
class ContextualSentenceGenerator extends stream.Transform {
    ...
    async _process(minibatch) {
        ...
        this._generator.generate(contexts, this._output.bind(this));
...
class MinibatchDialogueGenerator {
    constructor(sentenceGenerator, targetLanguage, dialogueAgent, options, minibatchIdx) {
        ...
        this._microbatchSize = Math.ceil(options.targetPruningSize * 0.5);
        ...
        this._completeDialogs = new ReservoirSampler(Math.ceil(this._minibatchSize * 500), this._rng);
    }

    _maybeAddCompleteDialog(dlg) {
        assert(dlg.turns.length > 0);
        this._completeDialogs.add(dlg.turns);
    }
    ...
    async nextTurn() {
        ...
        if (this._minibatchSize === 1) {
            ...
            for (let start = 0; start < partials.length; start += this._microbatchSize) {
                this._sentenceGenerator.generate(partials.slice(start, start+this._microbatchSize), (depth, derivation) => {
                    this._tryAddTurn(continuations, derivation);
                });
            }
        }
