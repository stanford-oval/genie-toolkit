...
class ContextualSentenceGenerator extends stream.Transform {
    ...
    _output(depth, derivation) {
        ...
        const program = derivation.value;
        ...
        id = context.id + '-' + depth + '000000000'.substring(0,9-id.length) + id;
        const flags = {
            synthetic: true,
            contextual: true,
        };
        return this.push({ depth, id, flags, preprocessed, context: context.code.join(' '), target_code: sequence.join(' ') });
    }
    ...
    async _process(minibatch) {
        ...
        const contexts = await Promise.all(minibatch.map(async (ex) => {
            const code = ex.target_code.split(' ');

            const entities = makeDummyEntities(ex.preprocessed);
            const program = ThingTalk.NNSyntax.fromNN(code, entities);
            await program.typecheck(this._generator.schemas, false);
            return new $runtime.Context(ex.id, code, program, entities);
        }));
