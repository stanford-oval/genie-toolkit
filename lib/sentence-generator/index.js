...
class SentenceGenerator extends events.EventEmitter {
    ...
    async initialize() {
        ...
        const $options = {
            ...
            compoundArrays: this._compoundArrays,
            whiteList: this._options.white_list ? this._options.white_list.toLowerCase().split(',') : null,
    ...
    _recordOutputParam(pname, ptype, arg) {
        ...
        if (pname in argNameOverrides)
            ...
            expansion = this._options.flags.schema_org ? makeArgCanonicals(arg.canonical, ptype) : [arg.canonical];

        if (arg.canonical !== expansion[0])
            console.log(`${pname}\t${expansion.join(', ')}`);
    ...
    async _loadDevice(device) {
        ...
        if (this._options.flags.schema_org) {
            ...
            await Promise.all([
                Promise.all(Object.values(classDef.queries).map((q) => {
                    if (this._options.whiteList && this._options.whiteList.includes(q.name.toLowerCase())) {
                        const ex = makeExampleFromQuery(q);
                        return this._loadTemplate(ex);
                    }
                    return null;
                })),
                Promise.all(Object.values(classDef.actions).map((a) => {
                    if (this._options.whiteList && this._options.whiteList.includes(a.name.toLowerCase())) {
                        const ex = makeExampleFromAction(a);
                        return this._loadTemplate(ex);
                    }
                    return null;
                })),
