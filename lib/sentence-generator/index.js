...
class SentenceGenerator extends events.EventEmitter {
    ...
    _recordType(type) {
        ...
        if (this._langPack.GRAMMATICAL_GENDERS.length > 0) {
            for (let gender of this._langPack.GRAMMATICAL_GENDERS) {
                ...
                if (type.isArray) {
                    this._grammar.addRule('out_param_Array__Any' + '_' + gender,  [new $runtime.NonTerminal('out_param_' + typestr + '_' + gender)],
                        $runtime.simpleCombine(identity));
                } else {
                    this._grammar.addRule('out_param_Any_' + gender,  [new $runtime.NonTerminal('out_param_' + typestr + '_' + gender)],
                        $runtime.simpleCombine(identity));
                }
            }
    ...
    _addOutParam(pname, typestr, cat, canonical) {
        let gender;
        [gender, canonical] = this._normalizeGender(canonical);

        if (cat === 'npv' || cat === 'apv') {
            // empty string has no gender, so we get rid of it quickly
        ...
        if (gender !== null) {
            this._grammar.declareSymbol('out_param_' + cat + '_' + gender);
            if (cat === 'npp') {
                for (let candidate of canonical)
                    this._grammar.addRule('out_param_' + typestr + '_' + gender, [candidate], $runtime.simpleCombine(() => new Ast.Value.VarRef(pname)));
            }
            for (let candidate of canonical)
                this._grammar.addRule('out_param_' + cat + '_' + gender, [candidate], $runtime.simpleCombine(() => new Ast.Value.VarRef(pname)));
        } else {
            this._grammar.declareSymbol('out_param_' + cat);
            if (cat === 'npp') {
                for (let candidate of canonical)
    ...
    async _loadTemplate(ex) {
        ...
        if (ex.type === 'program') {
            ...
            for (let pname in ex.args) {
                ...
                let pcanonical = clean(pname);
                args.push(new Ast.ArgumentDef(Ast.ArgDirection.IN_REQ, pname, ptype, { canonical: pcanonical }, {}));

                let gender;
                [gender, pcanonical] = this._normalizeGender(pcanonical);

                this._allParams.in.set(pname + '+' + ptype, [pname, [typeToStringSafe(ptype), pcanonical, gender]]);
    ...
    async _expandDataset(canonical, expander, rules) {
        ...
        await Promise.all(rules.map((rule) =>  {
            if (!rule.category.startsWith('thingpedia_query'))
                return;
