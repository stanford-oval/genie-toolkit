...
class SentenceGenerator {
    ...
    async _loadTemplate(ex) {
        ...
        // composable
        if (this._options.flags.turking && ex.type === 'action' && ex.value.invocation.selector.kind === 'org.thingpedia.builtin.thingengine.builtin')
            return;
        if (!this._options.flags.configure_actions && ex.type === 'action' && ex.value.invocation.selector.kind === 'org.thingpedia.builtin.thingengine.builtin'
            && (ex.value.invocation.channel === 'configure' || ex.value.invocation.channel === 'discover'))
            return;
        if (ex.type === 'action' && ex.value.invocation.selector.kind === 'org.thingpedia.builtin.thingengine.builtin' && ex.value.invocation.channel === 'say')
            return;
...
class ContextualSentenceGenerator extends stream.Transform {
    ...
    _output(depth, contextCode, contextEntities, contextId, derivation) {
        ...
        id = contextId + '-' + depth + '000000000'.substring(0,9-id.length) + id;
        const flags = {
            synthetic: true,
            contextual: true,
        };
        return this.push({ depth, id, flags, preprocessed, context: contextCode.join(' '), target_code: sequence.join(' ') });
    }

    async _process(contextExample) {
        if (!this._initialized) {
            await this._generator.initialize();
            this._initialized = true;
        }

        const contextCode = contextExample.target_code.split(' ');

        const entities = makeDummyEntities(contextExample.preprocessed);
        const contextProgram = ThingTalk.NNSyntax.fromNN(contextCode, entities);
        await contextProgram.typecheck(this._generator.schemas, false);

        const context = new $runtime.Context(contextCode, contextProgram);
        for (let [depth, derivation] of this._generator.generate(context))
            this._output(depth, contextCode, entities, contextExample.id, derivation);
    }

    _transform(contextExample, encoding, callback) {
        this._process(contextExample).then(callback, callback);
    }
