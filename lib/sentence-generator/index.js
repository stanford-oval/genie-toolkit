...
const { Context, Placeholder } = require('./derivations');
...
class SentenceGenerator extends events.EventEmitter {
    constructor(options) {
        ...
        this._algorithm = options.algorithm || 'bottomup';
    ...
    async initialize() {
        ...
        this._grammar = new ALGORITHMS[this._algorithm](this._options);
        ...
        for (let preprocessed of ex.preprocessed) {
            ...
            for (let chunk of chunks) {
                ...
                if (chunk.startsWith('$') && chunk !== '$$') {
                    ...
                    assert(param);
                    expansion.push(new Placeholder(param, opt));
                } else {
...
class ContextualSentenceGenerator extends stream.Transform {
    ...
    async _process(minibatch) {
        ...
        const contexts = await Promise.all(minibatch.map(async (contextCode) => {
            ...
            return new Context(code, program, entities);
        }));
