...
class SentenceGenerator extends events.EventEmitter {
    ...
    _recordOutputParam(pname, ptype, arg) {
        ...
        if (useGenie !== undefined && !useGenie)
            this._allParams.blacklist.add(key);
    ...
    async _loadTemplate(ex) {
        ...
        if (ex.type === 'query') {
            const human_entity_types = ['tt:contact', 'tt:username', 'org.wikidata:human', 'org.schema:Person'];
            ...
            await Promise.all([
                Promise.all(Object.values(classDef.queries).map(async (q) => {
                    if (this._options.whiteList && this._options.whiteList.includes(q.name.toLowerCase())) {
                        const id = this._entities[`org.schema:${q.name}`];
