...
class MinibatchDialogueGenerator {
    ...
    async init(initialSentences) {
        this._partialDialogs = await Promise.all(initialSentences.map(async (example) => {
            ...
            const entities = makeDummyEntities(example.preprocessed);
            const initState = await this._target.parse(code, entities, this._options);
            ...
            const dlg = [{
                ...
                user_target: initState.prettyprint()
            }];
            const context = this._dialogueAgent.execute(initState);
            return [context, dlg];
        }));
    ...
    async nextTurn() {
        ...
        for (let [, derivation] of this._sentenceGenerator.generate(contexts)) {
            ...
            const [agent, user] = preprocessed.split(' <sep> ');
            assert(agent);
            assert(user);

            const newContext = this._dialogAgent.execute(userState);
...
class DialogueGenerator extends stream.Transform {
    constructor(options) {
        super({ objectMode: true });
