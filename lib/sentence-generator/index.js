...
class SentenceGenerator extends events.EventEmitter {
    ...
    async _loadTemplate(ex) {
        ...
        for (let preprocessed of ex.preprocessed) {
            ...
            rules.push({ category: grammarCat, expansion, example: ex });
        }
        return rules;
    }
    ...
    async _expandDataset(canonical, expander, rules) {
        ...
        await Promise.all(rules.map((rule) =>  {
            ...
            const canonicalIndex = expander.expansion.indexOf(canonical);
            const preprocessed = expander.expansion.slice(0, canonicalIndex).concat(
                rule.expansion
            ).concat(
                expander.expansion.slice(canonicalIndex + 1)
            ).join(' ');
    ...
    async _loadDataset(dataset) {
        ...
        for (let channel in rules) {
            const canonical = canonicals[channel];
            for (let rule of rules[channel]) {
                if (rule.category !== 'thingpedia_query')
                    continue;
                if (!rule.expansion.includes(canonical))
                    continue;
