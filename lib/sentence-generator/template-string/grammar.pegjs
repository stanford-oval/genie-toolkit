...
maybe_choice = '{' first:template __ rest:('|' __ template __)* '}' {
    if (rest.length === 0)
        return first;
    return new Ast.Choice([first].concat(take(rest, 2)));
}
...
plural_variant = __ type:plural_type __ expansion:maybe_choice __ {
...
select_variant = __ type:identifier __ expansion:maybe_choice __ {
    return [type, expansion];
}
...
wrapped_placeholder = '${' __ param:identifier __ flag:('[' __ flag_spec __ ']')? __ opt:(':' __ identifier __)? '}' {
...
} / '${' __ param:qualified_name opt:(':' __ identifier __)? '}' {
