...
const ENTITIES = {
    ...
    QUOTED_STRING: (idx) => `str:QUOTED_STRING::${idx}:`,
    PATH_NAME: (idx) => `str:PATH_NAME::${idx}:`,
    URL: (idx) => `str:URL::${idx}`,
    PHONE_NUMBER: (idx) => `str:PHONE_NUMBER::${idx}:`,
    EMAIL_ADDRESS: (idx) => `str:EMAIL_ADDRESS::${idx}:`,
    USERNAME: (idx) => `str:USERNAME::${idx}:`,
    HASHTAG: (idx) => `str:HASHTAG::${idx}:`,
};
...
function makeDummyEntity(token) {
    ...
    if (entityType.startsWith('MEASURE_'))
        ...
        return { value: 'str:' + entityType.substring('GENERIC_'.length) + '::' + entityIndex, display: null };
