...
module.exports = class Dialog {
    ...
    handleGeneric(analyzer) {
        if (this.subdialog !== null) {
            return this.subdialog.handle(analyzer).then((handled) => {
                if (handled)
                    return true;
                else
                    return this._continueHandleGeneric(analyzer);
            })
        } else {
            return Q(this._continueHandleGeneric(analyzer));
        }
    }

    _continueHandleGeneric(analyzer) {
        if (analyzer.isSpecial) {
            ...
            case 'tt:root.special.failed':
                ...
                // (it will fall through to whatever dialog.handleFailed()
    ...
    handleFailed(command) {
        return this.handleGeneric(command);
    }
