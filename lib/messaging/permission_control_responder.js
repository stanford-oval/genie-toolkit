...
class ResponderFeed {
    ...
    _onIncomingMessage(message) {
        ...
        case 'install-rule':
            ...
            break;
        case 'remote-data':
            this._handleRemoteData(message, parsed);
            break;
    ...
    _handleRemoteData(message, parsed) {
    ...
    _handleInstallRule(message, parsed) {
        ...
        this._manager.installRuleReceived(this._feed.feedId, opId, principal, rule);
        Q.try(() => {
            return this._executor.installRule(principal, rule);
        }).then((result) => {
            this._manager.installRuleApproved(this._feed.feedId, opId);
            return this._feed.sendItem({
                version:1, ns: Config.RDF_BASE, op:'execute-reply',
                id: opId,
                error: null });
        }, (error) => {
            this._manager.installRuleDenied(this._feed.feedId, opId);
...
module.exports = class PermissionControlResponder {
    ...
    installRuleReceived(feedId, opId, principal, rule) {
        var tokens = new Set;
    ...
    subscribe(principal, token, callback) {
        ...
        if (this._deferredDataPackets[subid]) {
            this._deferredDataPackets[subid].forEach((data) => callback(data));
            delete this._deferredDataPackets[subid];
        }
    }
