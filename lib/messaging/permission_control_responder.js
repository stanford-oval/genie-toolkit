...
class ResponderFeed {
    ...
    _onIncomingMessage(feedId, message) {
        ...
        if (parsed.v !== CURRENT_VERSION || parsed.ns !== Config.RDF_BASE) {
    ...
    _handleInstallProgram(feedId, message, parsed) {
        ...
        this._manager.handleInstallProgram(principal, identity, uniqueId, code);
    }
...
module.exports = class PermissionControlResponder {
    ...
    _installRuleDenied(feedId, uniqueId) {
        let fullId = feedId + ':' + uniqueId;
        for (let subid of this._receivedInstallPrograms.keys()) {
            if (this._receivedInstallPrograms.get(subid) === fullId) {
    ...
    _typecheckProgram(code) {
        ...
        return ThingTalk.Grammar.parseAndTypecheck(code, this._schemaRetriever, true)
    }

    handleInstallProgram(principal, identity, uniqueId, code) {
        this._typecheckProgram(code).then((program) => {
            if (this._installProgramReceived(feedId, uniqueId, principal, identity, program))
                return this._executor.installProgram(principal, identity, program).then(() => true);
            else
                return false;
        }).then((result) => {
            if (result)
    ...
    _sendMessage(principal, op, msg) {
        return this._getFeed(principal).then((feed) => {
            return feed.open().then(() => {
                msg.v = CURRENT_VERSION;
                msg.ns = Config.RDF_BASE;
                return feed.sendItem(makeMessage(op, msg));
            });
        });
    }
    ...
    installProgramRemote(principal, identity, uniqueId, program) {
        return this._sendMessage(principal, {
            ...
            uuid: uniqueId,
            id: identity,
            c: ThingTalk.Ast.prettyprint(program, true)
        });
    }
}
