...
class ResponderFeed {
    ...
    _onIncomingMessage(feedId, message) {
        ...
        if (parsed.version !== CURRENT_VERSION) {//|| parsed.ns !== Config.RDF_BASE)
    ...
    _handleRemoteData(feedId, message, parsed) {
        ...
        try {
            ...
            var data = Protocol.params.unmarshal(parsed.data);
        } catch(e) {
    ...
    _handleInstallProgram(feedId, message, parsed) {
        ...
        Q.try(() => {
            ...
            program = ThingTalk.Grammar.parseAndTypecheck(code);
            this._manager.installProgramReceived(feedId, opId, principal, identity, program);
        }).then(() => {
            return this._executor.installProgram(principal, identity, program);
        }).then((result) => {
            this._manager.installProgramApproved(feedId, opId);
            return feed.sendItem({
                version:CURRENT_VERSION, ns: Config.RDF_BASE, op:'execute-reply',
                id: opId,
                error: null });
        }, (error) => {
            this._manager.installProgramDenied(feedId, opId);
...
module.exports = class PermissionControlResponder {
    constructor(executor, messaging, tierManager, devices) {
        this._executor = executor;
        this._messaging = messaging;
        ...
        this._receivedInstallPrograms = {};
    }

    installProgramReceived(feedId, opId, principal, identity, program) {
        ...
        for (let token of tokens) {
            ...
            this._receivedInstallPrograms[subid] = feedId + ':' + opId;
        }
    }

    installProgramApproved(feedId, opId) {
        let fullId = feedId + ':' + opId;
        for (let subid in this._receivedInstallPrograms) {
            if (this._receivedInstallPrograms[subid] === fullId) {
                console.log('Approved token ' + subid);
                setTimeout(() => {
                    delete this._receivedInstallPrograms[subid];
    ...
    handleData(principal, token, data) {
        ...
        if (callbacks.length === 0) {
            if (!(subid in this._receivedInstallPrograms))
                return;
            if (this._deferredDataPackets[subid]) {
    ...
    installProgramRemote(principal, identity, program) {
        ...
        return this._getFeed(principal).then((feed) => {
            ...
            defer.channelType = 'program';

            return feed.open().then(() => {
                return feed.sendItem({
                    version:2, ns: Config.RDF_BASE, op:'install-program',
                    id: opId,
                    identity: identity,
