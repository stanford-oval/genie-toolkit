...
class ResponderFeed {
    ...
    _onIncomingMessage(message) {
        ...
        case 'execute':
            ...
            break;
        case 'execute-reply':
            this._handleExecuteReply(message, parsed);
            break;
    ...
    _handleExecuteReply(message, parsed) {
        var feedId = this._feed.feedId;
        try {
            var opId = parsed.id;
            var result = parsed.result;
            var error = parsed.erorr;
        } catch(e) {
    ...
    _handleExecute(message, parsed) {
        ...
        this._executor.execute(principal, device, channel, channelType, args)
            .then((result) => {
                var marshalled;
